...
FORMULA_FILE="homebrew/smelter.rb"
...
SHA_DARWIN_ARM64=$(echo "$CHECKSUMS" | grep "smelter-darwin-arm64" | awk '{print $1}')
SHA_DARWIN_X64=$(echo "$CHECKSUMS" | grep "smelter-darwin-x64" | awk '{print $1}')
SHA_LINUX_ARM64=$(echo "$CHECKSUMS" | grep "smelter-linux-arm64" | awk '{print $1}')
SHA_LINUX_X64=$(echo "$CHECKSUMS" | grep "smelter-linux-x64" | awk '{print $1}')
...
validate_checksum "smelter-darwin-arm64" "$SHA_DARWIN_ARM64"
validate_checksum "smelter-darwin-x64" "$SHA_DARWIN_X64"
validate_checksum "smelter-linux-arm64" "$SHA_LINUX_ARM64"
validate_checksum "smelter-linux-x64" "$SHA_LINUX_X64"
...
# Darwin ARM64: line after smelter-darwin-arm64 URL
sed -i.bak '/smelter-darwin-arm64/{n;s/sha256 "[a-f0-9]\{64\}"/sha256 "'"${SHA_DARWIN_ARM64}"'"/;}' "$FORMULA_FILE"
# Darwin x64: line after smelter-darwin-x64 URL
sed -i.bak '/smelter-darwin-x64/{n;s/sha256 "[a-f0-9]\{64\}"/sha256 "'"${SHA_DARWIN_X64}"'"/;}' "$FORMULA_FILE"
# Linux ARM64: line after smelter-linux-arm64 URL
sed -i.bak '/smelter-linux-arm64/{n;s/sha256 "[a-f0-9]\{64\}"/sha256 "'"${SHA_LINUX_ARM64}"'"/;}' "$FORMULA_FILE"
# Linux x64: line after smelter-linux-x64 URL
sed -i.bak '/smelter-linux-x64/{n;s/sha256 "[a-f0-9]\{64\}"/sha256 "'"${SHA_LINUX_X64}"'"/;}' "$FORMULA_FILE"
