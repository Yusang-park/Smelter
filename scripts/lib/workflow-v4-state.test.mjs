...
test('live command modes explore and dobby create valid v0.4 state', () => {
  ...
  const dobby = createV4State({ taskId: 't-dobby', userMode: 'dobby' });
  assert.equal(dobby.task_type, 'freeform');
  assert.deepEqual(dobby.step_flow, ['INTENT', 'EXECUTE', 'HUMAN_CHECK', 'DONE']);
  assert.deepEqual(validateV4State(dobby), []);
});
