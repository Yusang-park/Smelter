...
echo "Building Smelter CLI v${VERSION} (commit: ${GIT_COMMIT})"

# Regenerate bundled defaults from .smelter/{commands,workflows}/defaults/ so the
...
if [ -n "$TARGET" ] && [ -n "$OUTFILE" ]; then
  ...
  mkdir -p "$DIST_DIR"
  TARGETS=(
    "bun-darwin-arm64:${DIST_DIR}/smelter-darwin-arm64"
    "bun-darwin-x64:${DIST_DIR}/smelter-darwin-x64"
    "bun-linux-x64:${DIST_DIR}/smelter-linux-x64"
    "bun-linux-arm64:${DIST_DIR}/smelter-linux-arm64"
  )
fi
