#!/usr/bin/env bun
/**
 * Smelter CLI - Run AI workflows from the command line
 *
 * Usage:
...
Examples:
  ...
  smelter workflow run smelter-explore "Explain the login flow"
  smelter workflow run smelter-brainstorm --cwd /path/to/repo "Add dark mode"
  smelter workflow run smelter-implement --branch feature-auth "Implement auth"
  smelter workflow run smelter-fix --no-worktree "Fix typo"
  smelter continue fix/issue-42 --workflow smelter-verify "Review the changes"
...
async function main(): Promise<number> {
  ...
  // `smelter --version` works the same as `smelter version` from any directory.
  ...
  try {
    ...
    if (requiresGitRepo) {
      ...
      if (!repoRoot) {
        ...
        console.error('The Smelter CLI must be run from within a git repository.');
    ...
    switch (command) {
      ...
      case 'chat': {
        ...
        if (!chatMessage) {
          console.error('Usage: smelter chat <message>');
          return 1;
        }
      ...
      case 'workflow':
        switch (subcommand) {
          ...
          case 'run': {
            ...
            if (!workflowName) {
              console.error('Usage: smelter workflow run <name> [message]');
              return 1;
            }
          ...
          case 'resume': {
            ...
            if (!resumeRunId) {
              console.error('Usage: smelter workflow resume <run-id>');
              return 1;
            }
          ...
          case 'abandon': {
            ...
            if (!abandonRunId) {
              console.error('Usage: smelter workflow abandon <run-id>');
              return 1;
            }
          ...
          case 'approve': {
            ...
            if (!approveRunId) {
              console.error('Usage: smelter workflow approve <run-id> [comment]');
              return 1;
            }
          ...
          case 'reject': {
            ...
            if (!rejectRunId) {
              console.error('Usage: smelter workflow reject <run-id> [reason]');
              return 1;
            }
            const rejectReason =
          ...
          case 'cleanup': {
            ...
            if (Number.isNaN(days) || days < 0) {
              console.error('Usage: smelter workflow cleanup [days]');
          ...
          case 'event': {
            ...
            if (!runId) {
              console.error(
                'Usage: smelter workflow event emit --run-id <uuid> --type <event-type>'
              );
      ...
      case 'complete': {
        ...
        if (branches.length === 0) {
          console.error('Usage: smelter complete <branch-name> [branch2 ...]');
          return 1;
        }
      ...
      case 'continue': {
        ...
        if (!continueBranch) {
          console.error('Usage: smelter continue <branch> [--workflow <name>] "instruction"');
          return 1;
        }
