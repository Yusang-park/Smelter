...
const GITHUB_REPO = 'Yusang-park/Smelter';
...
export async function serveCommand(opts: ServeOptions): Promise<number> {
  ...
  if (!BUNDLED_IS_BINARY) {
    console.error('Error: `smelter serve` is for compiled binaries only.');
...
async function downloadWebDist(version: string, targetDir: string): Promise<void> {
  const tarballUrl = `https://github.com/${GITHUB_REPO}/releases/download/v${version}/smelter-web.tar.gz`;
  ...
  const expectedHash = parseChecksum(checksumsText, 'smelter-web.tar.gz');
