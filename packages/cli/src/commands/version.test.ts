...
describe('versionCommand', () => {
  ...
  it('should output version and system info', async () => {
    ...
    // First call should contain "Smelter CLI" and version
    const firstCall = consoleSpy.mock.calls[0][0] as string;
    expect(firstCall).toContain('Smelter CLI');
  ...
  it('should output correct format for version line', async () => {
    ...
    const firstCall = consoleSpy.mock.calls[0][0] as string;
    // Format: "Smelter CLI v0.2.0"
    expect(firstCall).toMatch(/^Smelter CLI v\d+\.\d+\.\d+$/);
  });
