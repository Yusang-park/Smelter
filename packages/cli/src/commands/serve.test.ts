...
describe('parseChecksum', () => {
  ...
  it('should extract hash for matching filename', () => {
    const checksums = [
      `${'b'.repeat(64)}  smelter-linux-x64`,
      `${validHash}  smelter-web.tar.gz`,
      `${'c'.repeat(64)}  smelter-darwin-arm64`,
    ].join('\n');

    expect(parseChecksum(checksums, 'smelter-web.tar.gz')).toBe(validHash);
  });

  it('should handle single-space separator', () => {
    const checksums = `${validHash} smelter-web.tar.gz\n`;
    expect(parseChecksum(checksums, 'smelter-web.tar.gz')).toBe(validHash);
  });

  it('should throw for missing filename', () => {
    const checksums = `${validHash}  smelter-linux-x64\n`;
    expect(() => parseChecksum(checksums, 'smelter-web.tar.gz')).toThrow(
      'Checksum not found for smelter-web.tar.gz'
    );
  });

  it('should throw for empty checksums text', () => {
    expect(() => parseChecksum('', 'smelter-web.tar.gz')).toThrow('Checksum not found');
  });
  ...
  it('should throw for malformed hash (not 64 hex chars)', () => {
    const checksums = 'short_hash  smelter-web.tar.gz\n';
    expect(() => parseChecksum(checksums, 'smelter-web.tar.gz')).toThrow(
      'Malformed checksum entry for smelter-web.tar.gz'
    );
  });

  it('should throw for uppercase hex hash', () => {
    const checksums = `${'A'.repeat(64)}  smelter-web.tar.gz\n`;
    expect(() => parseChecksum(checksums, 'smelter-web.tar.gz')).toThrow(
      'Malformed checksum entry for smelter-web.tar.gz'
    );
  });
...
describe('serveCommand', () => {
  ...
  it('should reject in dev mode (non-binary)', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'Error: `smelter serve` is for compiled binaries only.'
    );
  });
