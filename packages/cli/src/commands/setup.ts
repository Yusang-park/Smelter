...
async function collectClaudeBinaryPath(): Promise<string | undefined> {
  ...
  note(
    'Compiled Smelter binaries need CLAUDE_BIN_PATH set to the Claude Code executable.\n' +
...
async function collectPlatforms(): Promise<SetupConfig['platforms']> {
  const platforms = await multiselect({
    message:
      'Which chat adapters do you want to connect? (all optional — Smelter works as CLI + skill without any)\n(↑↓ navigate, space select, enter confirm)',
    options: [
...
async function collectTelegramConfig(): Promise<TelegramConfig> {
  ...
  note(
    'Telegram Bot Setup\n\n' +
      ...
      '2. Send /newbot\n' +
      '3. Choose a display name (e.g., "My Smelter Bot")\n' +
...
async function collectBotDisplayName(): Promise<string> {
  const customName = await confirm({
    message: 'Do you want to set a custom bot display name? (Default: Smelter)',
  });
...
export function generateEnvContent(config: SetupConfig): string {
  ...
  // Header
  lines.push('# Smelter Configuration');
  lines.push('# Generated by `smelter setup`');
  lines.push('');
  ...
  // Bot Display Name
  if (config.botDisplayName !== 'Smelter') {
...
export async function setupCommand(options: SetupOptions): Promise<void> {
  ...
  // Offer to install the bundled workflow skill
  const shouldCopySkill = await confirm({
    message: 'Install the bundled workflow skill in your project? (recommended)',
    initialValue: true,
  });
  ...
  if (skillInstalledPath) {
    summaryLines.push('');
    summaryLines.push('Bundled workflow skill installed:');
  ...
  note(
    'To update Smelter:\n' +
      '  Homebrew:  brew upgrade yusang-park/smelter/smelter\n' +
      '  curl:      curl -fsSL https://raw.githubusercontent.com/Yusang-park/Smelter/main/scripts/install.sh | bash\n' +
      '  Docker:    docker pull ghcr.io/yusang-park/smelter:latest',
    'Update Instructions'
  );
