---
title: Configuration
description: Configure Smelter with API keys, assistants, and project settings.
...
|----------|----------|-------------|
| `CLAUDE_BIN_PATH` | Yes (binary builds) | Absolute path to the Claude Code SDK's `cli.js`. Required in compiled Smelter binaries unless `assistants.claude.claudeBinaryPath` is set. Dev mode (`bun run`) auto-resolves via `node_modules`. |
...
| `CODEX_BIN_PATH` | No | Absolute path to the Codex CLI binary. Overrides auto-detection in compiled Smelter builds. |
...
Create `.smelter/config.yaml` in your repository:
