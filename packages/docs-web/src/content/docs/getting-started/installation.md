---
title: Installation
description: Install Smelter on macOS, Linux, or Windows.
...
```bash
docker run --rm -v "$PWD:/workspace" ghcr.io/yusang-park/smelter:latest workflow list
```
...
Smelter orchestrates Claude Code; it does not bundle it. Install Claude Code separately:
...
Docker images (`ghcr.io/yusang-park/smelter`) ship with Claude Code pre-installed and
