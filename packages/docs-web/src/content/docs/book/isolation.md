...
description: How Smelter uses git worktrees to run multiple workflows in parallel without conflicts.
category: book
part: core-workflows
audience: [user]
sidebar:
...
You noticed in Chapter 4 that most workflows accept a `--branch` flag. That flag isn't just naming a branch — it's activating the **isolation** system, the feature that lets Smelter run multiple tasks simultaneously without them stepping on each other.

---

## Why Isolation Matters

Imagine you're running `smelter-fix-github-issue` on issue #42 and ask it to also start working on issue #43. Without isolation, both tasks share the same files in your repository. Task A edits `auth.ts`. Task B also edits `auth.ts`. You now have a conflict mid-run, and Smelter can't make sense of which changes belong to which task.
...
This is also how you can safely let Smelter run overnight. A workflow that takes an hour and makes dozens of changes does it all in isolation. When it's done, you review the PR, and if you don't like it, you just close it — nothing in your working directory was ever affected.
...
When Smelter creates a worktree for a workflow run, it lands here:
...
|-----------------|----------|
| `smelter workflow run <name> "..."` | Auto-generates a branch name; runs in an isolated worktree |
| `smelter workflow run <name> --branch my-branch "..."` | Uses your branch name; runs in an isolated worktree |
| `smelter workflow run <name> --no-worktree "..."` | No isolation; runs directly in your current directory |

**The default is isolation.** If you don't pass `--no-worktree`, Smelter creates a worktree for you.

Use `--no-worktree` only for tasks that don't modify code — questions, exploration, running `smelter-assist`. For anything that touches files, isolation is the right choice.
...
Worktrees accumulate over time. Smelter gives you a few commands to stay on top of them.
...
```bash
smelter isolation cleanup 14   # Remove worktrees older than 14 days
```
...
```bash
smelter complete fix/issue-42
```

This is the full lifecycle close-out. Run it after merging and you're back to a clean state.

> **Safety note**: Smelter won't remove a worktree that has uncommitted changes. If `cleanup` skips a worktree, check it with `smelter isolation list` before deleting manually.
...
**Clean up after merges.** Run `smelter complete <branch>` or `smelter isolation cleanup --merged` after a PR lands. A handful of worktrees is fine; dozens of stale ones get confusing.
...
Now that you understand how isolation protects your work, you're ready to start building. In [Chapter 6: Creating Your First Command →](/book/first-command/), you'll write the atomic unit of Smelter — the command file — from scratch.
