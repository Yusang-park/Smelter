...
describe('widgets', () => {
  ...
  describe('contextWidget', () => {
    ...
    it('should treat gpt-5.4 sessions as a 1M context even when stdin reports 200k', async () => {
      const ctx = createContext({
  ...
  describe('codex compact preset', () => {
    it('should swap Claude rate-limit widgets for the Codex usage widget in codex sessions', async () => {
      vi.spyOn(codexClient, 'isCodexInstalled').mockResolvedValue(true);
      vi.spyOn(codexClient, 'fetchCodexUsage').mockResolvedValue({
        model: 'gpt-5.4',
        planType: 'max',
        primary: { usedPercent: 12, resetAt: null },
        secondary: null,
      } as any);
      ...
      expect(lines[0]).not.toContain('7d-S');
    });
  });
