...
**Plan Mode 연동**: `/tasker`는 `EnterPlanMode` → 계획 작성 → `ExitPlanMode` 호출.
Dual-write: `~/.claude/plans/<name>.md` ↔ `.smelter/features/<slug>/task/plan.md`. 충돌 시 `.smelter/` 우선.
...
| `[Plan Mode: Enter]` / `[Plan Mode: Exit]` | /tasker command | Step 1-3 |
