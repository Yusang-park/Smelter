...
// ---------------------------------------------------------------------------
section('Layer 2b — High-confidence read-only VCS analysis intents');
// ---------------------------------------------------------------------------
{
  const dir = mkdtempSync(join(tmpdir(), 'mc-vcs-read-'));
  try {
    const stub = mkStub(dir, `export function classifyMode() { throw new Error('LLM should not be called for VCS analysis intent'); }`);
    process.env.SMELTER_MODE_CLASSIFIER_MODULE = stub;
    const { classify } = await freshImport();
    const r = classify('현재 브랜치가 main이랑 차이나는 내용들이 무엇인지? 확인요망.', { cwd: dir, sessionId: 'vcs1' });
    assert('branch-vs-main diff question → explore', r.mode, 'explore');
    assert('branch-vs-main trigger', r.trigger, 'local:vcs-analysis');
    assert('branch-vs-main is not passthrough', r.passthrough, undefined);

    const en = classify('check what this branch differs from main', { cwd: dir, sessionId: 'vcs2' });
    assert('English branch diff question → explore', en.mode, 'explore');
  } finally {
    delete process.env.SMELTER_MODE_CLASSIFIER_MODULE;
    rmSync(dir, { recursive: true, force: true });
  }
