...
const COMMAND_SKILLS = new Set(['fix', 'implement', 'infra', 'explore', 'verify', 'brainstorm']);
...
function isReviewReworkStage(stage) {
  return typeof stage === 'string' && /-review$/.test(stage);
}
...
function chainBlockReason(state, requested) {
  ...
  const producer = producerForState(state, currentStage);
  if (producer === requested && isReviewReworkStage(currentStage)) {
    return null;                                      // review can request producer rework before a verdict
  }
  if (producer === requested && isLastEventFail(events, currentStage)) {
    return null;                                      // fail-route to producer
  }
  const expectedProducer = producer && isReviewReworkStage(currentStage)
    ? `, '${producer}' (review rework)`
    : '';
  return `[Smelter chain] Skill('${requested}') violates the declared pipeline from current stage '${currentStage}'. Expected one of: '${currentStage}' (re-entry), '${expectedNext}' (forward on pass)${expectedProducer}, or 'workflow-human-check' (escape). Fix: dispatch the expected next skill, invoke the review's producer for rework, or invoke workflow-human-check to hand control back to the user.`;
}
