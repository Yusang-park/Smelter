...
test('H4c: Skill(implement) does not point active-feature at an unreadable state path', () => {
  const cwd = makeTempCwd();
  try {
    ...
    const r = runWithCwd({
      tool_name: 'Skill',
      tool_input: { skill: 'implement', args: 'blocked recovery' },
      session_id: 'sess-H4c',
    }, cwd, { NODE_OPTIONS: `--require=${preload}` });

    assert.equal(r.status, 0);
    assert.equal(
      existsSync(join(cwd, '.smt', 'state', 'active-feature-sess-H4c.json')),
      false,
      'command-skill recovery must not write a pointer unless the target state.json is readable',
    );
  } finally { rmSync(cwd, { recursive: true, force: true }); }
});
...
test('CE9b edge: review rework — back to producer allowed before explicit fail event', () => {
  const cwd = makeTempCwd();
  try {
    seedState(cwd, 'sess-CE9b', {
      current_stage: 'workflow-investigate-review',
      events: [{ t: new Date().toISOString(), skill: 'workflow-investigate', result: 'pass', declarer: 'hook' }],
    });
    const r = runWithCwd({ tool_name: 'Skill', tool_input: { skill: 'workflow-investigate' }, session_id: 'sess-CE9b' }, cwd);
    const out = parse(r.stdout);
    assert.equal(out.continue, true, 'review-stage producer rework must be allowed without requiring a fail event first');
  } finally { rmSync(cwd, { recursive: true, force: true }); }
});
