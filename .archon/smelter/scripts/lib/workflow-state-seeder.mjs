...
import { isReadableStateFile } from './state-file-readiness.mjs';
...
export function seedWorkflowState({
  ...
  try {
    ...
    if (!existsSync(statePath)) {
      ...
      stateReady = isReadableStateFile(statePath);
      if (!stateReady) {
        log(`State seed failed: unreadable state file at ${statePath}`);
        return null;
      }
    }

    if (!stateReady || !isReadableStateFile(statePath)) return null;
