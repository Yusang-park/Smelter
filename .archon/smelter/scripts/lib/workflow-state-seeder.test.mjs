...
test('SL1b: seedWorkflowState supports explore from skill-tool and ignores retired commands', async () => {
  const { seedWorkflowState } = await import(MODULE);
  const cwd = tmp();
  try {
    ...
    const blocked = seedWorkflowState({ directory: cwd, commandName: 'retired-freeform', args: 'manual edit', sessionId: 'sl1b-b', source: 'skill-tool' });
    assert.equal(blocked, null, 'retired commands must not seed a new workflow');
  } finally { rmSync(cwd, { recursive: true, force: true }); }
});
