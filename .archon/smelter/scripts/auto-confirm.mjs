...
import { readLastAssistantText, lastAssistantQuestionShape, classifyQuestionShape, readPendingAskUserQuestionAnswer, readLastToolErrorText } from './lib/transcript-reader.mjs';
...
export function detectSmelterBlockedToolError(text) {
  const s = String(text || '');
  if (!s) return false;
  const smelterBlock = /\[(?:SMELTER|Smelter phase|Smelter chain)\]/.test(s);
  const blockSignal = /(?:guard blocked|requires phase|current phase|phase skip blocked|violates the declared pipeline|Expected one of|source edit requires EXECUTE)/i.test(s);
  return smelterBlock && blockSignal;
}

function currentStageHasPass(state) {
  const stage = state?.current_stage;
  if (!stage || !Array.isArray(state?.events)) return false;
  return state.events.some((e) => e && e.skill === stage && e.result === 'pass');
}

function recoverySkillForBlockedTool(state, text) {
  const s = String(text || '');
  const currentFromError = s.match(/current stage '([^']+)'/)?.[1]
    || s.match(/Expected one of:\s*'([^']+)'/)?.[1];
  if (currentFromError) return currentFromError;

  const current = state?.current_stage;
  if (current && !currentStageHasPass(state)) return current;

  const allowed = Array.isArray(state?.allowed_skills) ? state.allowed_skills : [];
  if (/source edit requires EXECUTE|guard blocked .* code file/i.test(s) && allowed.includes('workflow-coding')) {
    return 'workflow-coding';
  }
  return current || pickNextStage(state);
}
...
function isHoldNativeAnswer(answer) {
  const text = [answer?.text, ...Object.values(answer?.answers || {})].join('\n');
  return /(^|\b)(hold|보류|중단|stop|pause)(\b|$)/i.test(text);
}

export function decide({ state, lastAssistantText, statePath, stageClassifier, questionShape, pendingAskUserQuestionAnswer = null, lastToolErrorText = '' }) {
  if (!state) return { action: 'no_state', reason: 'no active task state.json found' };

  if (pendingAskUserQuestionAnswer && !isHoldNativeAnswer(pendingAskUserQuestionAnswer)) {
    return {
      action: 'native_answer_continue',
      reason: 'native AskUserQuestion answer received; continue from the user decision',
      payload: { answer: pendingAskUserQuestionAnswer },
    };
  }
  ...
  if (detectSmelterBlockedToolError(lastToolErrorText)) {
    const skill = recoverySkillForBlockedTool(state, lastToolErrorText);
    if (skill) {
      return {
        action: 'enter_skill',
        reason: `continue: blocked tool call must recover through ${skill}, not stop after the hook error.`,
        payload: { skill, direction: skill === state?.current_stage ? 'enter' : 'back' },
      };
    }
  }
...
export function buildPromptInjection(decision, state, { subAgentModel = 'sonnet', lastAssistantText = '', questionShape = 'none' } = {}) {
  ...
  switch (decision.action) {
    ...
    case 'native_answer_continue': {
      const answer = decision.payload?.answer;
      const answerText = answer?.text || JSON.stringify(answer?.answers || {});
      return [
        header,
        '[NATIVE QUESTION ANSWER]',
        answerText,
        '',
        'Continue from this user answer now. Do NOT ask the same question again.',
        'If the answer requests a mode transition or extra verification, invoke the appropriate Skill(...) as the first tool call; otherwise execute the requested next step with tools.',
      ].join('\n');
    }
    case 'halt':
    case 'no_state':
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const lastAssistantText = input.last_assistant_text
    ...
    || '';
  const lastToolErrorText = input.last_tool_error_text
    || readLastToolErrorText(input.transcript_path || '')
    || '';
  const pendingAskUserQuestionAnswer = readPendingAskUserQuestionAnswer(input.transcript_path || '');
  ...
  let decision = decide({ state, lastAssistantText, statePath, stageClassifier, questionShape, pendingAskUserQuestionAnswer, lastToolErrorText });
