...
test('T2-V1: Write on code file with NO active workflow is blocked (v3.3 gate)', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(out.reason, /`\/(?:fix|implement|infra)\s+<description>`/);
  } finally { await rm(cwd, { recursive: true, force: true }); }
});
...
test('T2-F4: raw code-write block reason exposes only supported agent recovery skills', async () => {
