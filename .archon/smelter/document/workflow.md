...
> Archon-first runtime: Archon owns workflow sequencing and lifecycle control. Smelter's Claude Code hook registration no longer installs workflow blocking/controller hooks (`UserPromptSubmit`, `PreToolUse`, `PostToolUse`, or `Stop` workflow controllers), so local hook blocks cannot interrupt Archon-run workflows.
> Workflow topology remains documented in `modes/workflow.yaml` for Archon workflows and skill prompts; Claude Code hooks are support-only by default.

---

## v0.6 Entry Contract

Archon invokes Smelter workflows directly. The default Claude Code hook registration does not route commands/natural-language intent through `UserPromptSubmit`; command routing, task memory, and workflow state are owned by the Archon workflow run.
