...
- [x] v0.6-gate-5. **Active hook registration parity** — `settings.json` mirrors the current `hooks/hooks.json` support-hook set. Earlier critical workflow gate hooks were removed from active registration by `v0.6-archon-first-1`; `hook-registration.test.mjs` now fails if they reappear.
- [x] v0.6-archon-first-1. **Remove Claude Code workflow blockers from active registration** — `hooks/hooks.json` and `settings.json` no longer register Smelter `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, or `Stop` workflow controller hooks. Archon owns workflow sequencing; Smelter keeps only support hooks such as session start/end, permission handling, sub-agent tracking, and pre-compact behavior. `hook-registration.test.mjs` now asserts the blocking/controller scripts stay unregistered.
...
- [x] v3.3.7-1. **Writer-tool gate widened narrowly** — `scripts/tool-retry.mjs` now accepts `Write|Edit|MultiEdit|NotebookEdit` followed by end-of-string, a word boundary, or `(`. This preserves the non-writer guard while allowing UI-decorated tool names such as `Write(/Users/yusang/Smelter/.archon/smelter/scripts/lib/yellow-tag.test.mjs)`.
- [x] v3.3.7-2. **RED/GREEN regression** — `scripts/tool-retry.test.mjs` adds a failing case for `tool_name: "Write(/Users/yusang/Smelter/.archon/smelter/scripts/lib/yellow-tag.test.mjs)"` plus the canonical `File has not been read yet...` error. RED before the regex change, GREEN after. Existing negative guards for Bash echo and stdout-only text remain green.
