...
Backed by the Archon-native `smelter-implement` workflow. Pipeline: `full` for all implement work. `target_type` remains context for planning, not a state-machine branch.
...
- Iron Law #4 (no retry): failures route through the next Archon node or explicit rework, not hidden state mutation.
