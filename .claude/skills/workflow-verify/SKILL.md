...
bun run type-check       # or project-specific equivalent
bun run lint             # or project-specific equivalent
```

Record: typecheck errors, lint violations, and doc-sync issues.
...
Record the actual interface that users / callers / clients touch. Every required surface must later produce an artifact file under `$ARTIFACTS_DIR/e2e/`. Zero-artifact E2E runs fail in `workflow-e2e` / `workflow-e2e-review`.
...
Phase 3 hands off to the full `workflow-e2e` real-interface contract, including the Effect-vs-Ack rule. A run with artifacts but no effect evidence still fails downstream E2E review.
...
- Skill `pass`: `verify_report.md` exists with Phase 1, Phase 2, and Phase 3 E2E-scope sections.
...
| "I'll skip Phase 2 if Phase 1 passed" | Typecheck / lint / doc-sync catch different bugs than tests. |
