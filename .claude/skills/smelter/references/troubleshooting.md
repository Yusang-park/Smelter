...
```
~/.smelter/workspaces/<owner>/<repo>/logs/<run-id>.jsonl
```
...
Find the run ID from `smelter workflow status` (most recent run). Then:
...
```
~/.smelter/workspaces/<owner>/<repo>/artifacts/runs/<run-id>/
```
...
```bash
ls ~/.smelter/workspaces/<owner>/<repo>/artifacts/runs/<run-id>/
cat ~/.smelter/workspaces/<owner>/<repo>/artifacts/runs/<run-id>/issues/issue-42.md
```
...
A node references `$BASE_BRANCH` in its prompt, but neither git auto-detection nor `worktree.baseBranch` in `.smelter/config.yaml` produced a branch.

**Fix:**
1. Set `worktree.baseBranch: main` (or `dev`, or whatever) in `.smelter/config.yaml`.
2. Or pass `--from <branch>` on `smelter workflow run`.
...
Compiled-binary builds of Smelter no longer embed Claude Code / Codex — you install them separately and Smelter resolves the binary via env var or config.

**Fix (Claude):**
- Install: `curl -fsSL https://claude.ai/install.sh | bash` (or `npm install -g @anthropic-ai/claude-code`)
- Set `CLAUDE_BIN_PATH=/path/to/claude` in `~/.smelter/.env`, OR
- Set `assistants.claude.claudeBinaryPath: /absolute/path` in `.smelter/config.yaml`
...
See [smelter.diy/getting-started/installation/](https://smelter.diy/getting-started/installation/) for full platform-specific install paths.

### Workflow shows `running` for a long time but nothing happens

Three possibilities:

1. **The AI is actually working.** Check `~/.smelter/workspaces/<owner>/<repo>/logs/<run-id>.jsonl` — if you see recent `tool` or `assistant` events in the tail, it's fine. Wait.
2. **The server crashed and left an orphan row.** Server startup no longer auto-fails orphaned `running` rows (per the "No Autonomous Lifecycle Mutation" rule — `CLAUDE.md`). Transition it manually:
   ...
   - CLI: `smelter workflow abandon <run-id>` — marks the DB row cancelled without killing any subprocess. Right tool for orphans since the subprocess is already gone
   - Chat (Slack / Telegram / Web): `/workflow cancel <run-id>` — actively terminates the subprocess. Use for a still-live run that needs to be interrupted (there is no `smelter workflow cancel` CLI subcommand)
...
```bash
smelter workflow run my-workflow "original message"
# → "Resuming workflow — skipping N already-completed node(s)"
```

Use `--resume` only when you want to force-reuse the same worktree from a specific failed run. Use `smelter workflow resume <run-id>` to force a specific run ID.
...
User-level Claude plugin MCPs (e.g. `telegram`, `notion`) inherited from `~/.claude/` fail to connect in the headless subprocess. This is normal — they're not configured for Smelter's worktree context. Smelter filters these to debug logs (`dag.mcp_plugin_connection_suppressed`) and surfaces only workflow-configured MCP failures.
...
smelter workflow status --json | jq '.runs[]'
...
smelter validate commands my-command

# Dump the last 50 lines of a workflow's log
tail -n 50 ~/.smelter/workspaces/<owner>/<repo>/logs/<run-id>.jsonl | jq .

# Increase log verbosity (workflow run)
smelter workflow run my-workflow --verbose "..."
...
1. Run `smelter version` and note the version.
2. Run `smelter validate workflows <name>` and capture the output.
...
5. File an issue at https://github.com/coleam00/Smelter/issues with version, validate output, log tail, and the YAML.
