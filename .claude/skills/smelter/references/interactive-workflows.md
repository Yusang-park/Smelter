...
- `smelter-piv-loop` — Plan-Implement-Validate with iterative feedback
- `smelter-interactive-prd` — Guided PRD creation with approval gates
...
Check `smelter workflow status` periodically. When status changes to `paused`, the workflow is waiting for user input.
...
```bash
# Find the log file
find ~/.smelter/workspaces -name "<run-id>.jsonl" 2>/dev/null
...
```bash
smelter workflow approve <run-id> "<user's exact response>"
```
...
smelter workflow approve <run-id> "your feedback or answers here"

# Reject (cancels the workflow)
smelter workflow reject <run-id> "reason for rejection"
```

## Troubleshooting

- **Workflow shows `running` for a long time**: The AI is doing research/implementation. Be patient — check again in a few minutes.
- **Log file not found**: The log is at `~/.smelter/workspaces/<owner>/<repo>/logs/<run-id>.jsonl`
- **User wants to cancel**: Run `smelter workflow reject <run-id>` to stop at an approval gate, or `smelter workflow abandon <run-id>` to mark the run cancelled without killing any subprocess. To actively terminate a still-live subprocess, use the chat slash command `/workflow cancel <run-id>` on the platform that started it — there is no `smelter workflow cancel` CLI subcommand
