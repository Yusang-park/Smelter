...
Smelter workflows use a DAG (Directed Acyclic Graph) format: nodes with explicit dependency edges. Independent nodes run in parallel, conditions enable routing, and data flows between nodes via `$nodeId.output`. This is the only workflow format — there are no other workflow types.
...
|-------|------|-------------|
| `name` | string (required) | Workflow identifier (used in `smelter workflow run <name>`) |
| `description` | string (required) | Human-readable summary. Used for routing; see [Workflow Description Best Practices](https://smelter.diy/guides/authoring-workflows/#workflow-description-best-practices) |
| `provider` | string | AI provider (e.g. `claude`, `codex`, `pi`). Default: from `.smelter/config.yaml` |
...
Other worktree config (`baseBranch`, `copyFiles`, `initSubmodules`, `path`) lives in `.smelter/config.yaml`, not the workflow YAML — see `references/repo-init.md`.
...
| `sandbox` | object | OS-level filesystem/network restrictions. Nested `network` / `filesystem` sub-objects — see [smelter.diy/guides/authoring-workflows/#claude-sdk-advanced-options](https://smelter.diy/guides/authoring-workflows/#claude-sdk-advanced-options) for the full schema. Layers on top of worktree isolation |
...
### Command Node
Runs a command file from `.smelter/commands/`:
```yaml
- id: investigate
  command: investigate-issue         # Loads .smelter/commands/investigate-issue.md
```
...
```yaml
- id: analyze
  script: analyze-metrics           # Resolves .smelter/scripts/analyze-metrics.py
...
- **Named script resolution**: `<cwd>/.smelter/scripts/` (wins) → `~/.smelter/scripts/`. 1-level subfolder grouping allowed. Extension determines runtime (`.ts`/`.js` → `bun`, `.py` → `uv`) and MUST match the declared `runtime:`
- **Dispatch**:
...
```bash
smelter workflow run my-workflow --resume
```
...
4. User runs `smelter workflow approve <run-id> "<their feedback>"` (or replies naturally in chat platforms).
...
smelter workflow approve <run-id> --comment "looks good"
smelter workflow reject <run-id>
smelter workflow reject <run-id> --reason "plan needs more test coverage"
...
When a cancel node runs, Smelter:
...
Use `--json` for machine-readable output. Use `smelter validate commands <name>` to validate individual command files.
...
- Named scripts must exist in `.smelter/scripts/` or `~/.smelter/scripts/` with extension matching declared runtime
