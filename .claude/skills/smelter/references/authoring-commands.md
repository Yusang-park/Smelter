...
```
<repoRoot>/.smelter/commands/     # 1. Repo-scoped (wins)
├── my-command.md                #    Custom command for this repo
├── smelter-assist.md             #    Overrides the bundled smelter-assist
└── triage/                      #    Subfolders allowed, 1 level deep
    └── review.md                #    Resolves as 'review', not 'triage/review'

~/.smelter/commands/              # 2. Home-scoped (user-level, shared across all repos)
├── review-checklist.md          #    Personal helper available in every repo
└── pr-style-guide.md

<bundled defaults>                # 3. Shipped with Smelter (smelter-assist, etc.)
```
...
When a workflow references `command: my-command`, Smelter searches in this order:

1. `<repoRoot>/.smelter/commands/my-command.md` (repo scope)
2. `~/.smelter/commands/my-command.md` (home scope — shared across every repo on the machine)
3. Bundled defaults (shipped with Smelter)

First match wins. To override a bundled command, drop a file with the same name at either scope. To override a home-scoped command for a specific repo, drop a file with the same name in that repo's `.smelter/commands/`.

> **Web UI note**: Home-scoped commands appear in the workflow builder's node palette under a dedicated "Global (~/.smelter/commands/)" section, distinct from project and bundled entries.
