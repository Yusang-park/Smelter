...
One-page lookup for Smelter workflow parameters: which field works on which node type, how to pick the right parameter for a given intent, and the gotchas that don't fail loudly.
...
| Give a node external tools                       | `mcp: .smelter/mcp/server.json`                               |
...
See [smelter.diy/guides/authoring-workflows/#inline-sub-agents](https://smelter.diy/guides/authoring-workflows/#inline-sub-agents) for a worked end-to-end example.
...
| Repo initialization, `.smelter/config.yaml` schema | `repo-init.md`                                                         |
...
| Full schema reference                            | [smelter.diy/reference/configuration/](https://smelter.diy/reference/configuration/) |
...
2. Validate before running: `smelter validate workflows <name>`.
