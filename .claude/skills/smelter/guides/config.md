# Smelter Configuration Guide

Interactive guide for viewing and modifying Smelter configuration. Use this when the user wants to change, view, or understand their config — at any point, not just during initial setup.
...
Options:
  1. "Repo config" (Recommended) — Settings for a specific repository (.smelter/config.yaml in the repo)
  2. "Global config" — User-wide settings (~/.smelter/config.yaml)
  3. "Both" — Review and modify both configs
```

## Step 2: Load Current Config

### For Global Config (~/.smelter/config.yaml)

Read the file:
```bash
cat ~/.smelter/config.yaml
```

If it doesn't exist, tell the user it will be auto-created on first Smelter run with defaults. Offer to create it now.

### For Repo Config (<repo>/.smelter/config.yaml)

First determine the target repo. If the current working directory is a git repo (and not the Smelter source repo), use it. Otherwise ask:
...
Then read the file:
```bash
cat <target-repo>/.smelter/config.yaml
```

If it doesn't exist, tell the user: "No repo config found. I can create one — or Smelter will use defaults." Offer to create it.
...
Format example:
```
Current repo config (.smelter/config.yaml):
...
Options:
  1. "Bot name" — Display name shown in messages (current: {value}, default: Smelter)
...
After writing, confirm: "Updated {path}. Changes take effect on the next Smelter invocation (no restart needed for CLI)."
...
### Global Config (~/.smelter/config.yaml)

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `botName` | string | `Smelter` | Bot display name shown in messages |
...
| `paths.workspaces` | string | `~/.smelter/workspaces` | Directory for cloned repositories |
| `paths.worktrees` | string | `~/.smelter/worktrees` | Directory for git worktrees |
| `concurrency.maxConversations` | number | `10` | Maximum concurrent AI conversations |

### Repo Config (<repo>/.smelter/config.yaml)
...
2. **Repo config** (`.smelter/config.yaml` in repo) — project-specific
3. **Global config** (`~/.smelter/config.yaml`) — user-wide preferences
...
| `SMELTER_HOME` | Base directory for all Smelter paths |
