...
The user is inside the **remote-coding-agent** repository — that's how they have access to this skill. The Smelter repo path is the current working directory. Store it as `<smelter-repo>`.

## Step 1: Ask for Target Repo

**IMPORTANT**: The target repo is the user's own project — **never** the remote-coding-agent (Smelter) repo itself. Do not suggest or offer the current directory as an option.

Use **AskUserQuestion** with a single question:

```
Header: "Target repo"
Question: "What is the path to the repository you want to work on using Smelter? (This should be your own project, not the Smelter repo.)"
Options:
  1. "Clone from GitHub" — user provides a GitHub URL; clone it to ~/.smelter/workspaces/
```
...
```bash
smelter-repo-path=$(pwd)
mkdir -p ~/.smelter/workspaces
cd ~/.smelter/workspaces && git clone <url>
```
...
1. `cd <smelter-repo> && ~/.bun/bin/bun install` (or `bun install` if bun was already in PATH)
2. `cd <smelter-repo>/packages/cli && ~/.bun/bin/bun link` (or `bun link`)
3. Verify: `smelter version`
4. Check Claude is installed: `which claude`, then `claude /login` if needed

> **Note — Claude Code binary path.** Smelter does not bundle Claude Code. In compiled Smelter binaries (quick install, Homebrew), the Claude Code SDK needs `CLAUDE_BIN_PATH` set to the absolute path of its `cli.js`. The `smelter setup` wizard in Step 4 auto-detects this via `npm root -g` and writes it to `~/.smelter/.env` — no manual action needed in the typical case. Source installs (`bun run`) don't need this; the SDK finds `cli.js` via `node_modules` automatically.

## Step 4: Configure Credentials

Smelter loads infrastructure config (database, tokens) from two smelter-owned files — `~/.smelter/.env` (user scope) and `<cwd>/.smelter/.env` (repo scope, overrides user). The project's own `<cwd>/.env` is stripped at boot so it cannot leak into Smelter; `smelter setup` never writes to it.
...
**CRITICAL**: Do NOT run `smelter setup` directly via Bash — it requires interactive input that I cannot provide. The `--spawn` flag attempts to open a new terminal window where the user can interact directly.
...
>
> By default it saves to `~/.smelter/.env` (user scope). Re-run with `smelter setup --scope project` to write `<repo>/.smelter/.env` instead (project overrides user for this repo). Existing values are preserved — a timestamped backup is written before every rewrite."
...
| Issue | Cause | Fix |
|-------|-------|-----|
| Shows `sqlite` but expected `postgresql` | `~/.smelter/.env` missing or no DATABASE_URL | Run `smelter setup` again in your terminal |
...
Bundled default commands and workflows are loaded automatically at runtime from the Smelter installation — nothing needs to be copied into the target repo.

Tell the user:
- "Default commands and workflows are loaded automatically at runtime — no files are added to your repo."
- "To browse defaults, look in `<smelter-repo>/.smelter/commands/defaults/` and `<smelter-repo>/.smelter/workflows/defaults/`."
- "To customize a default, copy the specific file into your repo's `.smelter/commands/` or `.smelter/workflows/` directory with the same filename. Your repo version takes priority over the bundled default."

## Step 7: Start the Server (non-CLI platforms only)

**Skip if "CLI only" was selected.**

After all platform tokens are in `.env`, read and follow `guides/server.md` to start the server from the smelter repo.
...
```bash
cd <target-repo> && smelter workflow list
```

If the CLI is working, also run:

```bash
cd <target-repo> && smelter workflow run smelter-assist "Say hello"
```
...
| Error | Cause | Fix |
|-------|-------|-----|
| `smelter: command not found` | CLI not linked | Re-run `cd <smelter-repo>/packages/cli && bun link` |
| `Not a git repository` | Not in a git repo | `cd` to the target repo root |
| `No workflows found` | Missing `.smelter/workflows/` | Default workflows load automatically — check `smelter version` works first |
| Auth errors | Claude not authenticated | Run `claude /login` |
| `relation "remote_agent_*" does not exist` | DATABASE_URL missing or tables not created | Ensure `~/.smelter/.env` has DATABASE_URL and run migrations |
| `Database: sqlite` but expected PostgreSQL | `~/.smelter/.env` missing DATABASE_URL | Add DATABASE_URL to `~/.smelter/.env` |
...
```
Header: "Smelter skill"
Question: "Would you like to copy the Smelter skill into your target repo so Claude Code can invoke Smelter workflows from there?"
Options:
  1. "Yes, copy the skill" — Copies .claude/skills/smelter/ into the target repo. This will appear in git unless you gitignore it.
  2. "No, skip" — You can always run Smelter workflows from the Smelter repo instead.
```
...
cp -r <smelter-repo>/.claude/skills/smelter <target-repo>/.claude/skills/smelter
```
...
4. The smelter skill is now loaded — ask Claude to run workflows, fix issues, review PRs, etc.

Example first command in the target repo:
```
"Use smelter to fix issue #1"
```

Tell the user:
- Default commands and workflows load automatically at runtime — nothing was added to your repo
- `.claude/skills/smelter/` — skill for Claude Code integration
- To customize a default, copy the specific file from `<smelter-repo>/.smelter/commands/defaults/` or `<smelter-repo>/.smelter/workflows/defaults/` into your repo's `.smelter/commands/` or `.smelter/workflows/` with the same filename

**Important**: End the summary with this message:

> "If you want to configure advanced options later — like changing the default AI assistant, customizing worktree behavior, or adjusting which files get copied into isolated environments — just ask me to help you with 'smelter config' and I'll walk you through it."

The end state: user is in their target repo with the Smelter skill available, defaults loaded at runtime without polluting the repo, using Claude Code as the interface.
...
Smelter's env model is scoped by directory ownership: `.smelter/` is smelter-owned, anything else belongs to you.

| Path | Stripped at boot? | Smelter loads? | `smelter setup` writes? |
|------|-------------------|---------------|------------------------|
| `<cwd>/.env` | **yes** (safety guard) | never | never |
| `<cwd>/.smelter/.env` | no | yes (project scope, overrides user scope) | yes iff `--scope project` |
| `~/.smelter/.env` | no | yes (user scope) | yes iff `--scope home` (default) |

**Which should I use?**

- `~/.smelter/.env` — defaults that apply everywhere (your personal `SLACK_WEBHOOK`, `DATABASE_URL`, bot tokens).
- `<cwd>/.smelter/.env` — per-project overrides (different webhook per repo, different DB per environment).
- `<cwd>/.env` — your app's env file; smelter strips these keys at boot so nothing leaks between your app and smelter.

`smelter setup` writes to exactly one smelter-owned file chosen by `--scope` (default `home`), merges into existing content so user-added keys survive, and writes a timestamped backup before every rewrite. Use `--force` to opt into wholesale overwrite (backup still written).
...
|----------|-------|---------|
| `~/.smelter/config.yaml` | Global | Default AI assistant, streaming modes, concurrency |
| `<repo>/.smelter/config.yaml` | Per-repo | AI assistant, worktree settings, commands config |
...
| `commands.folder` | string | `.smelter/commands` | Custom command folder path (relative to repo root) |
