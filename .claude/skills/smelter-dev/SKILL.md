...
name: smelter-dev
description: |
  The PRIMARY development workflow for the Smelter project (remote-coding-agent).
  Use this skill instead of any PRP skills when working on Smelter code.
  ...
  IMPLEMENT   — "implement the plan", "execute .claude/smelter/plans/auth.plan.md",
  ...
  NOT for: Running Smelter CLI workflows in worktrees (use /smelter instead).
...
- **Artifacts**: !`ls .claude/smelter/ 2>/dev/null || echo "none yet"`
- **Active plans**: !`ls .claude/smelter/plans/*.plan.md 2>/dev/null | head -5 || echo "none"`
...
All artifacts go to `.claude/smelter/`. Create subdirectories as needed on first use.
