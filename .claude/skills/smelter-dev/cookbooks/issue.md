...
- {For bugs}: `/smelter-dev debug #{number}` to investigate
- {For features}: `/smelter-dev prd` to write requirements, or `/smelter-dev plan` to design
```
...
```
/smelter-dev issue                              # Use recent conversation context
/smelter-dev report this to gh                  # Same — infer from context
/smelter-dev log this bug in github             # Bug from context
/smelter-dev create a feature request for X     # Feature with description
/smelter-dev gh issue: streaming breaks on large responses  # Bug with description
```
