...
2. **If no path**: Look for the most recent `.plan.md` in `.claude/smelter/plans/` (excluding `completed/`)
3. **If no plans found**: Tell the user and suggest `/smelter-dev plan`
...
Save to `.claude/smelter/reports/{slug}-report.md` using the plan's slug.
...
```bash
mkdir -p .claude/smelter/plans/completed
mv {plan-path} .claude/smelter/plans/completed/
```
...
To continue: `/smelter-dev plan {prd-path}`
```

**Next steps**:
- To commit: `/smelter-dev commit`
- To create PR: `/smelter-dev pr`
- To review: `/smelter-dev review`
