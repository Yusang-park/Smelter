...
If `--follow-up` and an existing research file on this topic exists in `.claude/smelter/research/`:
...
Save to `.claude/smelter/research/{date}-{slug}.md`.
...
**Next steps**:
- To dig deeper: `/smelter-dev research --follow-up {topic}`
- For strategic/external research: `/smelter-dev investigate {topic}`
- To write requirements: `/smelter-dev prd {topic}`
