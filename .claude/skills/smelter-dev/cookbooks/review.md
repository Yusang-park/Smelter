...
Check `.claude/smelter/` for artifacts related to this work:

```bash
ls .claude/smelter/reports/ 2>/dev/null
ls .claude/smelter/plans/completed/ 2>/dev/null
ls .claude/smelter/issues/ 2>/dev/null
ls .claude/smelter/debug/ 2>/dev/null
```
...
Save to `.claude/smelter/reviews/{date}-pr-{number}.md` (or `{date}-{slug}.md` for local changes).
...
gh pr review {NUMBER} --approve --body-file .claude/smelter/reviews/{filename}.md

# If --request-changes OR high issues:
gh pr review {NUMBER} --request-changes --body-file .claude/smelter/reviews/{filename}.md

# Otherwise just comment:
gh pr comment {NUMBER} --body-file .claude/smelter/reviews/{filename}.md
```
...
Suggest: `/smelter-dev review {pr-number}` to self-review before requesting human review.
