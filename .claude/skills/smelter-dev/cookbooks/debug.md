...
**If from a GitHub issue**: Save to `.claude/smelter/issues/issue-{number}.md`
**Otherwise**: Save to `.claude/smelter/debug/{date}-{slug}.md`
...
**Next steps**:
- For complex fixes: `/smelter-dev plan` (create a plan from the RCA)
- For simple fixes: `/smelter-dev implement` (implement directly)
