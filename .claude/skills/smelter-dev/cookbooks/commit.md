...
- Files committed
- Suggest next step: `/smelter-dev pr` to create a pull request

### Examples

```
/smelter-dev commit                          # All changes
/smelter-dev commit typescript files         # *.ts only
/smelter-dev commit except package-lock      # Exclude specific
/smelter-dev commit only the new files       # Untracked only
/smelter-dev commit staged                   # Already-staged only
/smelter-dev commit the auth refactor        # Context-based targeting
```
