...
Skill-level `pass` requires two documented rounds and all rounds result `pass`. Record the rounds in `investigation-review.md`.
...
| Thought | Reality |
|---------|---------|
| "The investigation looks fine, skip round 2" | Run both rounds; downstream nodes rely on this artifact. |
...
All rounds are recorded in `investigation-review.md`. Skill-level `pass` is declared only when both rounds are present and both results are `pass`.
