...
Declaring pass with fewer than two recorded rounds or only one reviewer is invalid. Record evidence in `agent-review.md`.
...
| "Only round 1 is needed for small diffs" | Run both rounds; downstream E2E relies on the artifact. |
...
All rounds are recorded in `agent-review.md`. Skill-level `pass` is declared only when both rounds are present and both results are `pass`.
