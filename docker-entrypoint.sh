...
mkdir -p /.smelter/workspaces /.smelter/worktrees

# Determine if we need to use gosu for privilege dropping
if [ "$(id -u)" = "0" ]; then
  # Running as root: fix volume permissions, then drop to appuser
  if ! chown -Rh appuser:appuser /.smelter 2>/dev/null; then
    echo "ERROR: Failed to fix ownership of /.smelter — volume may be read-only or mounted with incompatible options" >&2
    exit 1
  fi
  # /home/appuser is persisted to a named volume (or bind-mounted via
  # SMELTER_USER_HOME) so Claude/Codex/Pi config, ~/.gitconfig, shell history,
  # and other user-specific state survive rebuilds. On bind mounts, host UIDs
  # don't map to appuser (1001), so fix ownership the same way we do /.smelter.
...
# These leak in but have no effect (SMELTER_HOME is overridden to /.smelter by source;
# SMELTER_DATA is a host-side compose substitution token, never read by the container).
if [ -n "${SMELTER_HOME:-}" ]; then
  echo "[smelter] SMELTER_HOME=${SMELTER_HOME} ignored in Docker (container home is fixed at /.smelter)" >&2
fi
if [ -n "${SMELTER_DATA:-}" ]; then
  echo "[smelter] SMELTER_DATA=${SMELTER_DATA} is a host-side compose token; not read inside the container" >&2
fi

# Register all git repositories under /.smelter as safe directories.
...
# every boot.
find /.smelter -name ".git" -prune -print 2>/dev/null | while IFS= read -r git_dir; do
