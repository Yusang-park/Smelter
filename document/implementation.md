---
title: Smelter Workflow v0.55 — Implementation Status
status: in_progress
started: 2026-04-20
updated: 2026-04-29
---

# Smelter v0.55 Implementation Status
...
- [x] v0.55-dobby-1. **Dobby is user-only** — `keyword-detector.mjs`, `pre-tool-enforcer.mjs`, `state-contract-injector.mjs`, and `workflow-state-seeder.mjs` no longer expose or honor agent-owned `Skill(dobby)` recovery. `/dobby` still works as an explicit user slash command, and the injected command skill is allowed only when that slash command already seeded active dobby state.
...
- [x] v0.4.1-8. **Explicit dobby escape reseed** — superseded by v0.55-dobby-1: agent-owned `Skill(dobby)` no longer creates a fresh freeform feature over active non-dobby workflows. User-typed `/dobby` remains the explicit no-pipeline escape hatch.
...
Raw code-write block messages no longer tell the agent to ask the user to type a slash command. Recovery wording now tells the agent to invoke `Skill(fix|implement|infra)` itself to seed active state, or continue only after an existing active workflow pointer is present. `/dobby` is kept user-only and intentionally omitted from agent-owned recovery guidance.
...
- [x] P26-1. `scripts/keyword-detector.mjs::createSkillInvocation` changed from hard “first tool must be Skill” language to a routing gate: read/search is allowed for clarification, but source edits and mutating commands require the selected `Skill(fix|implement|infra)` first; `/dobby` is not offered as agent-selected correction.
