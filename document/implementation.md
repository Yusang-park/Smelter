...
- [x] v0.51-dobby-1. **Dobby is user-only** — `keyword-detector.mjs`, `pre-tool-enforcer.mjs`, `state-contract-injector.mjs`, and `workflow-state-seeder.mjs` no longer expose or honor agent-owned `Skill(dobby)` recovery. `/dobby` still works as an explicit user slash command, and the injected command skill is allowed only when that slash command already seeded active dobby state.
...
- [x] v0.4.1-8. **Explicit dobby escape reseed** — superseded by v0.51-dobby-1: agent-owned `Skill(dobby)` no longer creates a fresh freeform feature over active non-dobby workflows. User-typed `/dobby` remains the explicit no-pipeline escape hatch.
