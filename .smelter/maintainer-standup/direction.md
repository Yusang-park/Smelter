...
- **Workflow-driven.** Reproducible AI execution chains defined as YAML DAGs in `.smelter/workflows/`. Workflows run in isolated git worktrees by default.
- **Type-safe.** Strict TypeScript everywhere. No `any` without justification.
- **Composable.** Scripts in `.smelter/scripts/`, commands in `.smelter/commands/`, workflows compose them.
...
- **Not a replacement for the AI coding agent itself.** Smelter orchestrates Claude Code / Codex / Pi — it doesn't reimplement them.
...
- **Not a workflow marketplace.** Bundled workflows are reference patterns; Smelter is not aiming to be a hub for third-party workflow distribution.
...
- Add a "What Smelter IS" or "is NOT" line when a PR triage forces a direction call.
