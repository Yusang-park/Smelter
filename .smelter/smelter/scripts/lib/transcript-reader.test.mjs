...
test('readPendingAskUserQuestionAnswer: latest native question tool_result is returned', () => {
  const { dir, path } = writeTranscript([
    userText('start'),
    { role: 'assistant', content: [{ type: 'tool_use', id: 'q1', name: 'AskUserQuestion', input: {} }] },
    {
      role: 'user',
      content: [{ type: 'tool_result', tool_use_id: 'q1', content: 'User has answered your questions: "Next?"="Proceed with E2E". You can now continue with the user\'s answers in mind.' }],
      toolUseResult: { answers: { 'Next?': 'Proceed with E2E' } },
    },
  ]);
  try {
    const answer = readPendingAskUserQuestionAnswer(path);
    assert.equal(answer.answers['Next?'], 'Proceed with E2E');
    assert.match(answer.text, /Proceed with E2E/);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});

test('readPendingAskUserQuestionAnswer: returns null after assistant responds', () => {
  const { dir, path } = writeTranscript([
    userText('start'),
    { role: 'assistant', content: [{ type: 'tool_use', id: 'q1', name: 'AskUserQuestion', input: {} }] },
    userToolResult('q1', { content: 'User has answered your questions: "Next?"="Proceed".' }),
    assistantText('I will proceed now.'),
  ]);
  try {
    assert.equal(readPendingAskUserQuestionAnswer(path), null);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
...
test('readLastToolErrorText: returns latest current-turn errored tool_result', () => {
  const { dir, path } = writeTranscript([
    userText('old turn'),
    userToolResult('old_tool', { is_error: true, content: 'old failure' }),
    userText('current turn'),
    assistantSkillCall('toolu_1', 'workflow-write-test'),
    userToolResult('toolu_1', { is_error: true, content: '[Smelter phase] Skill requires phase TEST_DESIGN' }),
  ]);
  try {
    assert.match(readLastToolErrorText(path), /TEST_DESIGN/);
    assert.doesNotMatch(readLastToolErrorText(path), /old failure/);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});

test('readLastToolErrorText: returns empty when latest current turn has no tool error', () => {
  const { dir, path } = writeTranscript([
    userText('current turn'),
    assistantSkillCall('toolu_1', 'workflow-investigate-review'),
    userToolResult('toolu_1', { is_error: false, content: 'ok' }),
  ]);
  try {
    assert.equal(readLastToolErrorText(path), '');
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
