...
// Return the latest errored tool_result text from the current user turn.
// Stop hooks often fire immediately after a PreToolUse hook blocks Edit/Write
// or Skill; that failure is recorded as a user-role tool_result, not assistant
// prose, so readLastAssistantText() cannot see it.
export function readLastToolErrorText(transcriptPath) {
  const entries = loadTranscriptEntries(transcriptPath);
...
export function readPendingAskUserQuestionAnswer(transcriptPath) {
  const entries = loadTranscriptEntries(transcriptPath);
  ...
  for (let i = entries.length - 1; i >= 0; i--) {
    ...
    const answers = msg.toolUseResult && typeof msg.toolUseResult === 'object'
      ? msg.toolUseResult.answers
      : null;
    const hasNativeAnswer = text.includes('User has answered your questions:')
      || text.includes("User answered Claude's questions:")
    ...
    if (isUserTextEntry(msg)) return null;
  }
