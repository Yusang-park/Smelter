...
const MODES_WHITELIST = Object.freeze([
  'brainstorm', 'fix', 'infra', 'explore', 'verify', 'implement',
]);
...
function classifyHighConfidenceReadOnlyIntent(input) {
  const text = stripTerminalPunctuation(input);
  if (!text) return null;

  const writeAction = /(?:고쳐|수정|구현|추가|변경|적용|반영|\b(?:fix|repair|implement|add|change|update|apply)\b)/i;
  if (writeAction.test(text)) return null;

  const vcsSubject = /(?:현재\s*)?(?:브랜치|branch|main|master|HEAD|커밋|commit|diff|변경\s*내용|차이(?:나|점)?|differs?|difference|changed?)/i;
  const readIntent = /(?:무엇|뭐|어떤|확인(?:요망|해|해줘|좀)?|알려|보여|요약|분석|조회|check|what|show|summari[sz]e|inspect|analy[sz]e)/i;
  const compareIntent = /(?:랑|와|과|대비|비교|from|against|vs\.?|versus|between)/i;
  if (vcsSubject.test(text) && (readIntent.test(text) || compareIntent.test(text))) {
    return {
      mode: 'explore',
      trigger: 'local:vcs-analysis',
      overridden: false,
      target_type: null,
      exempt: null,
...
export function classify(input, { cwd = process.cwd(), sessionId = '' } = {}) {
  ...
  const localReadOnlyIntent = classifyHighConfidenceReadOnlyIntent(trimmed);
  if (localReadOnlyIntent) return { schema_version: 2, ...localReadOnlyIntent };
