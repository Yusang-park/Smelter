...
import { isReadableStateFile } from './lib/state-file-readiness.mjs';
...
function extractExplicitHarnessCommand(prompt) {
  ...
  // (TPP6).
  const match = trimmed.match(/^\/(brainstorm|fix|infra|explore|implement|verify|cancel|queue)\b(?:[:\s-]*([\s\S]*))?$/i);
...
function seedWorkflowState(directory, commandName, prompt, sessionId, args = '', chainedModes = null, source = 'magic', preClassification = null) {
  ...
  try {
    ...
    if (!existsSync(stateJsonPath)) {
      ...
      stateReady = isReadableStateFile(stateJsonPath);
      if (!stateReady) printTag(`State seed failed: unreadable state file at ${stateJsonPath}`);
    }

    if (!stateReady || !isReadableStateFile(stateJsonPath)) return;
  ...
  try {
    ...
    if (!detected) {
      ...
      if (classification.intent === 'command' && classification.command) {
        const validCommands = ['brainstorm', 'fix', 'explore', 'implement', 'verify', 'cancel', 'queue'];
