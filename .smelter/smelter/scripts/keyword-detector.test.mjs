...
test('branch-vs-main diff request seeds explore, not verify', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'kwd-vcs-analysis-'));
  try {
    const out = runDetector({
      cwd,
      prompt: '현재 브랜치가 main이랑 차이나는 내용들이 무엇인지? 확인요망.',
      sessionId: 'vcs-analysis',
    });
    assert.equal(out.continue, true);
    assert.match(out.hookSpecificOutput?.additionalContext ?? '', /Skill: explore/);
    assert.doesNotMatch(out.hookSpecificOutput?.additionalContext ?? '', /Skill: verify/);

    const statePath = findStateFile(cwd);
    assert.ok(statePath, 'state file must be seeded for read-only workflow analysis');
    const state = JSON.parse(readFileSync(statePath, 'utf8'));
...
test('agent mode-correction prompt exposes only write recovery skills', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'kwd-write-recovery-'));
  try {
    const out = runDetector({ cwd, prompt: 'select만 적용.', sessionId: 'write-recovery' });
