...
// The retry hook must still recognize these as file-writer tools.
{
  ...
  const res = runHook({
    cwd: dir,
    session_id: 's-name',
    tool_name: 'Write(/Users/yusang/Smelter/.smelter/smelter/scripts/lib/yellow-tag.test.mjs)',
    tool_input: { file_path: '/Users/yusang/Smelter/.smelter/smelter/scripts/lib/yellow-tag.test.mjs', content: 'x' },
    tool_output: { error: 'File has not been read yet. Read it first before writing to it.', exit_code: 1 },
  }, dir);
  const out = JSON.parse(res.stdout);
