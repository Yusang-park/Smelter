...
/**
 * Global Claude Code hook wrapper for Smelter workflow hook guidance.
 *
 * Smelter's original hooks are node-level SDK responses in workflow YAML, not
 * standalone Claude settings commands. This wrapper mirrors the default Smelter
...
function handlePreToolUse(toolName) {
  ...
  emitAdditionalContext(
    'PreToolUse',
    [
      'Smelter global hook test: before writing, verify this file is part of the active plan or a direct dependency of the planned target.',
...
function handlePostToolUse(toolName) {
  if (/^(Write|Edit)$/.test(toolName)) {
    emit({
      systemMessage: [
        'Smelter global hook test: you just modified a file.',
  ...
  if (toolName === 'Read') {
    emitAdditionalContext(
      'PostToolUse',
      [
        'Smelter global hook test: for the file just read, assess single responsibility, cognitive load, abstraction value, and dependency direction.',
  ...
  if (toolName === 'Bash') {
    emitAdditionalContext(
      'PostToolUse',
      [
        'Smelter global hook test: check the exit code and diagnose failures before retrying.',
