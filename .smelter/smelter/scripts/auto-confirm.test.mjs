...
test('blocked Skill tool error re-enters current stage without requiring assistant question text', () => {
  const s = baseState({ mode: 'implement', current_stage: 'workflow-investigate-review', events: [passEvent('workflow-investigate')], completed_stages: ['workflow-investigate'] });
  const d = decide({
    state: s,
    lastAssistantText: '',
    questionShape: 'none',
    lastToolErrorText: "[Smelter chain] Skill('workflow-implementation-plan') violates the declared pipeline from current stage 'workflow-investigate-review'. Expected one of: 'workflow-investigate-review' (re-entry), 'workflow-tasker' (forward on pass), 'workflow-investigate' (review rework), or 'workflow-human-check' (escape).",
  });
  assert.equal(d.action, 'enter_skill');
  assert.equal(d.payload.skill, 'workflow-investigate-review');
  assert.equal(d.payload.direction, 'enter');
});
test('blocked source edit tool error routes to workflow-coding when current stage already passed', () => {
  const s = baseState({ mode: 'fix', current_stage: 'workflow-e2e-review', events: [passEvent('workflow-e2e-review')], completed_stages: ['workflow-e2e-review'] });
  const d = decide({
    state: s,
    lastAssistantText: '',
    questionShape: 'none',
    lastToolErrorText: '[SMELTER] v0.4 guard blocked Edit on code file: hooks/useSubscription.ts\nsource edit requires EXECUTE phase, current phase=DISCOVERY',
  });
  assert.equal(d.action, 'enter_skill');
  assert.equal(d.payload.skill, 'workflow-coding');
  assert.equal(d.payload.direction, 'back');
});
test('detectSmelterBlockedToolError recognizes phase and chain hook failures', () => {
  assert.equal(detectSmelterBlockedToolError('[Smelter phase] Skill requires phase TEST_DESIGN; phase skip blocked.'), true);
  assert.equal(detectSmelterBlockedToolError('[Smelter chain] violates the declared pipeline from current stage'), true);
  assert.equal(detectSmelterBlockedToolError('ordinary TypeScript failure'), false);
});
...
test('native AskUserQuestion answer after explore review pass → exit 2 + continuation queue', () => {
  const dir = mkdtempSync(join(tmpdir(), 'ac-v2-'));
  try {
    const featDir = join(dir, '.smt', 'features', 'nq', 'task');
    mkdirSync(featDir, { recursive: true });
    const statePath = join(featDir, 'nq.state.json');
    const state = baseState({
      mode: 'explore',
      current_stage: 'workflow-investigate-review',
      events: [passEvent('workflow-investigate-review')],
    });
    state.allowed_skills = ['workflow-investigate', 'workflow-investigate-review'];
    writeState(statePath, state);
    mkdirSync(join(dir, '.smt', 'state'), { recursive: true });
    const sessionId = 'sess-native-answer';
    writeFileSync(
      join(dir, '.smt', 'state', `active-feature-${sessionId}.json`),
      JSON.stringify({ slug: 'nq', session_id: sessionId }),
    );
    const transcriptPath = join(dir, 'transcript.jsonl');
    const transcript = [
      { role: 'assistant', content: [{ type: 'text', text: 'DB result found. 어떻게 진행?' }] },
      { role: 'assistant', content: [{ type: 'tool_use', id: 'q1', name: 'AskUserQuestion', input: {} }] },
      {
        role: 'user',
        content: [{ type: 'tool_result', tool_use_id: 'q1', content: 'User has answered your questions: "Next?"="추정하지말고 E2E로 impersonate 해봐". You can now continue with the user\'s answers in mind.' }],
        toolUseResult: { answers: { 'Next?': '추정하지말고 E2E로 impersonate 해봐' } },
      },
    ].map(e => JSON.stringify(e)).join('\n') + '\n';
    writeFileSync(transcriptPath, transcript, 'utf-8');

    const payload = JSON.stringify({ cwd: dir, session_id: sessionId, transcript_path: transcriptPath });
    const res = spawnSync('node', [HOOK], { input: payload, encoding: 'utf-8' });
    assert.equal(res.status, 2, `expected queue block, got ${res.status}\nstderr: ${res.stderr}\nstdout: ${res.stdout}`);
    const queuePath = join(dir, '.smt', 'state', `auto-confirm-queue-${sessionId}.json`);
    const queue = JSON.parse(readFileSync(queuePath, 'utf-8'));
    assert.equal(queue.action, 'native_answer_continue');
    assert.match(queue.additionalContext, /NATIVE QUESTION ANSWER/);
    assert.match(queue.additionalContext, /E2E로 impersonate/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
