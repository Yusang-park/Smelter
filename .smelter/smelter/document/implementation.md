---
title: Smelter Workflow v0.6 — Implementation Status
status: in_progress
started: 2026-04-20
updated: 2026-05-13
---

# Smelter v0.6 Implementation Status

> Source of truth: `document/workflow.md`.
> Principle: sequential execution, respect dependencies, no omissions.

## Schema v0.6 — prompt-injected workflow entry (2026-05-08)

- [x] v0.6-entry-1. **No mandatory state seeding at entry** — `keyword-detector.mjs` no longer creates `.state.json` or legacy `workflow.json` during `UserPromptSubmit`. It injects mode, target type, exemptions, chained modes, and allowed skills into prompt context.
- [x] v0.6-entry-3. **Prompt pointer replaces mandatory state** — `pre-tool-enforcer.mjs` allows code-file edits with an active write/infra/freeform prompt pointer even when `.state.json` is absent, while still blocking when no active workflow pointer exists. Legacy state-aware guards still run when a state file is present.
- [x] v0.6-plan-1. **Concise plan artifact** — entry-created `plan.md` now records only frontmatter and `## Request`; empty `Plan`, `Wiki Links`, and `Risks` sections were removed.
- [x] v0.6-gate-2. **Pointer-only commit gate** — active v0.6 write/infra/freeform prompt pointers without `.state.json` still block `git commit` until `workflow-human-check` passes, preserving the terminal human-review gate.
- [x] v0.6-gate-3. **Root-stable pointers** — `keyword-detector.mjs` resolves the nearest parent `.smt` root before sticky-workflow lookup/seeding, preventing nested working directories from forking `.smt`. The commit gate uses the same scoped-then-unscoped pointer fallback as source-edit gating.
- [x] v0.6-gate-4. **Readable-state pointer guard** — when the legacy entry-state path is present, `keyword-detector.mjs` and `workflow-state-seeder.mjs` write `active-feature-<session>.json` only after the target `.state.json` is readable JSON. Unreadable or directory-shadowed state paths fail closed instead of stranding RED recording on a missing state.
- [x] v0.6-gate-5. **Active hook registration parity** — `settings.json` mirrors the current `hooks/hooks.json` support-hook set. Earlier critical workflow gate hooks were removed from active registration by `v0.6-smelter-first-1`; `hook-registration.test.mjs` now fails if they reappear.
- [x] v0.6-smelter-first-1. **Remove Claude Code workflow blockers from active registration** — `hooks/hooks.json` and `settings.json` no longer register Smelter `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, or `Stop` workflow controller hooks. Smelter owns workflow sequencing; Smelter keeps only support hooks such as session start/end, permission handling, sub-agent tracking, and pre-compact behavior. `hook-registration.test.mjs` now asserts the blocking/controller scripts stay unregistered.
- [x] v0.6-routing-1. **VCS diff analysis routes to explore** — branch-vs-main/read-only diff questions such as “현재 브랜치가 main이랑 차이나는 내용” are classified locally as `/explore`, not `/verify`, so agents inspect git history/diff instead of entering verification mode.
- [x] v0.6-test-1. **Model settings test isolation** — `hud-summary-trigger.mjs` honors `CLAUDE_CODEX_SETTINGS_PATH`, and wrapper/model-picker tests use temporary settings files so parallel `npm test` runs do not race on the real shared settings file.

## Schema v0.55 — executable specs and compact fix/tasker records (2026-04-29)

- [x] v0.55-semble-1. **Semble MCP replaces Serena** — `.mcp.json` now launches MinishLab Semble via `uvx --from semble[mcp] semble`; `settings.json` allows `mcp__semble`; Serena ignore entries and stage-gate mutating MCP special cases were removed. `pretool-stage-gate.test.mjs` covers the retired Serena tool no longer being treated as a mutating MCP gate.

`/fix` now records a compact task checklist before coding without returning to verbose Markdown planning. The checklist keeps the file-based workflow contract while reducing tasker token spend. `fix_simple` remains the direct lightweight path for text/design surfaces.

- [x] v0.55-tasker-1. **Fix pipeline records scope** — `modes/workflow.yaml` routes the default `fix` pipeline through `workflow-tasker` and `workflow-tasker-review` before coding/test execution; `fix_simple` skips tasker stages and runs investigate → coding → e2e → human-check.
- [x] v0.55-tasker-2. **Compact artifact contract** — `tasks.md` uses frontmatter plus seven one-line checkboxes. `queue` may stay unchecked for downstream execution; `goal`, `approach`, `works`, `omissions`, `verified`, and `team_runtime` must be checked.
- [x] v0.55-tasker-3. **Review artifact contract** — `tasks-review.md` requires pass/fail/reshape, `consensus >= 0.95` for pass, and checked works/omissions/verified/side-effects/decision entries.
- [x] v0.55-tasker-4. **Regression coverage** — `task-artifact-contract.test.mjs`, `skill-stage-transition.test.mjs`, and `workflow-loader.test.mjs` cover compact validation, stage completion blocking, and `/fix` pipeline ordering.
- [x] v0.55-transition-1. **Workflow-declared transition adoption** — `modes/workflow.yaml → transition_adoptions` now declares cross-mode stage reuse. `workflow-transition-adoption.mjs` is a generic policy executor used by hooks, so mode-specific reuse such as `/explore → /fix|/implement` is edited in workflow config rather than scattered across hook branches.
- [x] v0.55-routing-1. **Broader UserPromptSubmit write pre-routing** — `mode-classifier.mjs` now routes high-confidence feature/UI-surface modification prompts such as `AI panel 기능 수정해줘` and `특정 기능 수정` to `/implement` before the LLM layer. Bug/error-signaled prompts stay on the normal classifier path so `/fix` routing is preserved. Regression coverage added to `mode-classifier.test.mjs` and `keyword-detector.test.mjs`.
- [x] v0.55-routing-2. **Focused design `/fix` routing** — `surface-extraction.mjs` recognizes focused visual slash-argument hints (`border`, `outline`, `ring`, `테두리`) as `target_type=design`, including ASCII tokens followed by Korean postpositions such as `border가`. `critic-watchdog.mjs` R05 now reuses `hasTddEntryEvidence()` so `tdd_adopted` and TDD-exempt design/text states are accepted consistently with the phase gate. Regression coverage added to `keyword-detector.test.mjs` and `critic-watchdog.test.mjs`.
- [x] v0.55-write-test-1. **Executable specification contract** — `workflow-write-test` now frames RED tests as executable specifications using Specification by Example / BDD / ATDD principles: concrete domain values, Given/When/Then or Arrange/Act/Assert shape, observable outcomes, and no implementation-detail coupling. `skill-prompt-budget.test.mjs` covers the skill prompt contract.
- [x] v0.55-e2e-visual-1. **UI visual pass gate** — `workflow-e2e` now requires bounding-box/viewport assertions for UI layout or position requirements, and `critic-watchdog.mjs` R18 blocks UI `e2e-review.md` pass verdicts unless the review records opened screenshot/video evidence with `opened:`, `observed:`, `expected:`, and viewport/bounding-box comparison. Regression coverage added to `critic-watchdog.test.mjs` and `skill-prompt-budget.test.mjs`.
- [x] v0.55-phase-block-1. **No halt on phase-block questions** — `auto-confirm.mjs` detects non-human-check “blocked by current phase / requires phase” questions and routes back to `workflow-coding` instead of letting the session stop on a user-facing “how to proceed?” prompt. Regression coverage added to `auto-confirm.test.mjs`.
- [x] v0.55-watchdog-r07-1. **Canonical artifact directory exemption** — `critic-watchdog.mjs` R07 now treats canonical workflow artifacts under both `.smt/features/<slug>/task/` and `.smt/features/<slug>/artifacts/` as in-scope, preventing `investigation-review.md` recovery writes from being misclassified as plan queue scope leaks. Regression coverage added to `critic-watchdog.test.mjs`.
- [x] v0.55-human-check-1. **Issue-first human decisions** — `workflow-human-check` now inspects verification/review artifacts before completion choices. If `/verify` or any other mode surfaces unresolved findings, missing evidence, `MEDIUM+` risks, unchecked acceptance items, or blockers, it must ask a problem-focused native `AskUserQuestion` with `rework` / `hold` / `upgrade` only. Commit/PR completion choices are shown only after findings are resolved. Regression coverage added to `skill-prompt-budget.test.mjs`.
- [x] v0.55-explore-native-1. **Explore exit native question enforcement** — `/explore` exit options now require native `AskUserQuestion` with no plain-text fallback. `auto-confirm.mjs` rejects plain mode-choice tables/lists after `workflow-investigate-review` passes and re-injects a native-question directive; `commands/explore.md` carries the mandatory prompt wording. Regression coverage added to `auto-confirm.test.mjs` and `skill-prompt-budget.test.mjs`.
- [x] v0.55-goal-passthrough-1. **External `/goal` passthrough** — `keyword-detector.mjs` and `mode-classifier.mjs` now fail closed for host-level `/goal ...` prompts, preventing trailing natural language such as `확인후 수정` from accidentally seeding `/implement` and replacing the active workflow. Regression coverage added to `keyword-detector.test.mjs` and `mode-classifier.test.mjs`.
...
The v0.4 guard now distinguishes test-file writes from product-source writes. `workflow-write-test` (`TEST_DESIGN`) may create or edit `*.test.*`, `*.spec.*`, or files under `test/`, `tests/`, `spec/`, and `e2e/` while `write_test` is allowed. Product source still requires `EXECUTE` plus executable specification RED evidence (`test_red`, `tdd_adopted`, `tdd_exempt`, or legacy RED `test_cycles[]`), and test files are blocked during `EXECUTE` so implementation does not mutate tests mid-coding.
...
Prior implementation wrote codex/claude mode state to a single cwd-scoped file, `.smt/state/model-mode.json`. When `claude-codex` and plain `claude` ran in the same project directory, the second wrapper overwrote the first's state; hooks reading that file then reported the wrong mode to the earlier session, and the downstream symptom was Claude Code's auto-permission classifier returning `gpt-5.4 is temporarily unavailable` in the plain window. This phase session-scopes the state file so concurrent windows never collide.

- [x] v3.3.7-1. **Path scoping** — `codex-for-claude-code/scripts/set-model-mode.mjs` widens `getModelModeStatePath(cwd, sessionId)` to require a `sanitizeSessionId`-valid sid (throws on missing/unsafe) and returns `.smt/state/model-mode-${sid}.json`. `applyCodexMode(settings, sessionId)` / `applyClaudeMode(settings, cwd, sessionId)` / `setModelMode(mode, cwd, sessionId)` thread the sid through. Module-level `statePath` const removed.
- [x] v3.3.7-2. **Wrapper sid lifecycle** — `codex-for-claude-code/scripts/claude-wrapper.mjs` reuses an inbound `SMELTER_SESSION_ID` (sanitized) or coins a fresh `randomUUID()` at `main()` start, injects it into the child env for codex launches, and explicitly blanks it on plain-claude launches. Pre-launch it (a) unlinks the legacy unscoped `.smt/state/model-mode.json` one-time and (b) sweeps stale `model-mode-*.json` older than 24h, skipping the current sid. Exit/SIGINT/SIGTERM/SIGHUP cleanup unlinks the scoped file alongside `clearModelCache()`.
- [x] v3.3.7-3. **Reader sanitization** — `scripts/lib/subagent-classifier.mjs`, `scripts/statusline-hud.mjs`, `scripts/hud-summary-trigger.mjs`, `scripts/pre-compact.mjs` all import `sanitizeSessionId` from `scripts/auto-confirm.mjs`, resolve `process.env.SMELTER_SESSION_ID` before path construction, and short-circuit to the existing settings/env fallback when sid is absent or unsanitary. `statusline-hud` drops the home-dir `~/.smt/state/model-mode.json` fallback (dead code after scoping; `SMELTER_MODEL_MODE === 'codex'` env safety net still detects codex mode without the file).
- [x] v3.3.7-4. **Path-traversal guard** — `sanitizeSessionId` (regex `/^[A-Za-z0-9_-]+$/`) gates every sid entry point (writer + 4 readers). A hostile or misconfigured `SMELTER_SESSION_ID=/../foo` degrades to null rather than constructing a traversal path.
- [x] v3.3.7-5. **TDD** — `scripts/test-model-mode-session-isolation.mjs` (new, 10 cases: 2 happy, 2 boundary, 2 error, 2 edge, 2 integration) RED before impl, GREEN after. Existing tests updated with sid fixtures: `scripts/test-model-mode-switch.mjs` (TEST_SESSION_ID + `process.chdir(tempCwd)` so writes land in the temp dir), `scripts/test-pre-compact.mjs` (scoped filename + `SMELTER_SESSION_ID` env), `scripts/test-statusline-hud.mjs` (scoped filename + env), `scripts/test-model-picker-config.mjs` (scoped filename).
- [x] v3.3.7-6. **Docs** — this phase + `document/workflow.md` §11 (model-mode section) describes session-scoped state key (`SMELTER_SESSION_ID`), filename pattern (`.smt/state/model-mode-${sid}.json`), sanitizer contract, sweep TTL.
...
## Phase v3.3.6 — ungate `.json` / `.jsonc` from code-file workflow gate (2026-04-23)

Declarative JSON configs (package.json, tsconfig.json, plugin.json, etc.) behaved more like YAML/toml than like source code: trivial version bumps, path tweaks, and dependency pins were forced through a full `/fix` or `/implement` workflow with no reviewer-catchable logic change. This phase moves `.json` / `.jsonc` into the same "not gated" bucket as `.yaml` / `.yml`, matching user directive ("postUserTool에서 json은 제외해주면 돼").

- [x] v3.3.6-1. **CODE_FILE_EXTENSIONS trim** — `scripts/pre-tool-enforcer.mjs:199-213` no longer contains `'json'` / `'jsonc'`. Inline comment explains the declarative-config rationale and clarifies that protected state files under `.smt/` are still blocked by `PROTECTED_RE` (state-file regex runs independently of CODE_FILE_EXTENSIONS).
- [x] v3.3.6-2. **Tests** — `scripts/pre-tool-enforcer.test.mjs` adds T2-V10 (`.json` ungated), T2-V11 (`.jsonc` ungated), T2-V12 (regression: `.state.json` under `.smt/` stays blocked). RED before the trim, GREEN after. Existing T2-V1/V3/V4/V9 (code-file gate still enforced for `.ts` etc.) remain GREEN.
- [x] v3.3.6-3. **Docs** — `CLAUDE.md` v3.3 section: `.json` / `.jsonc` moved from Gated to Not-Gated list; protected-path caveat explicitly stated so readers do not misread the change as opening a state-file bypass.

## Phase v3.3.5 — Read-before-Write reminder + file-not-read retry fallback (2026-04-23)

Claude Code's harness rejects `Write`/`Edit` of a file that has not been `Read` in the current session with the error `File has not been read yet. Read it first before writing to it.`. Previously the `tool-retry.mjs` PostToolUse hook caught this error and asked the agent to Read-then-retry, but (a) the prompt fired only AFTER the failure, and (b) the instruction was a single line that a busy agent could skim past — resulting in repeated file-not-read failures on the same path. This phase adds a preventive PreToolUse nudge AND an escalated fallback message when the initial auto-retry does not recover.

- [x] v3.3.5-1. **Preventive prompt** — `scripts/pre-tool-enforcer.mjs` normal-flow path appends a `Read-first reminder:` block to `additionalContext` whenever `tool_name ∈ {Write, Edit}` and `existsSync(file_path)` is true. The reminder echoes the exact path, states the harness contract, and explicitly exempts new-file writes (no file → no Read needed). Coexists with the existing `Writing:`/`Editing:` tool description.
- [x] v3.3.5-2. **Retry fallback** — `scripts/tool-retry.mjs::buildRetryInstruction` gains a `currentCount = 0` parameter. For action `read-then-retry`, `currentCount >= 1` emits a multi-step `FALLBACK (escalated...)` message enumerating steps (1)…(4): explicit Read call, verify, re-issue, spurious-error recovery. Attempt 1 keeps the original single-line form to avoid bloating every call. The main loop passes the pre-increment `current` into the builder.
...
- [x] v3.3.7-1. **Writer-tool gate widened narrowly** — `scripts/tool-retry.mjs` now accepts `Write|Edit|MultiEdit|NotebookEdit` followed by end-of-string, a word boundary, or `(`. This preserves the non-writer guard while allowing UI-decorated tool names such as `Write(/Users/yusang/Smelter/.smelter/smelter/scripts/lib/yellow-tag.test.mjs)`.
- [x] v3.3.7-2. **RED/GREEN regression** — `scripts/tool-retry.test.mjs` adds a failing case for `tool_name: "Write(/Users/yusang/Smelter/.smelter/smelter/scripts/lib/yellow-tag.test.mjs)"` plus the canonical `File has not been read yet...` error. RED before the regex change, GREEN after. Existing negative guards for Bash echo and stdout-only text remain green.
- [x] v3.3.7-3. **Verification** — `node scripts/tool-retry.test.mjs` and `node scripts/lib/yellow-tag.test.mjs` pass after the fix.

## Phase v3.3.9 — prevent stale-read writer failures (2026-04-29)

User report: `Write(/Users/yusang/code/chartmetric/admini-tool/client/src/lib/ai-chat-session.ts)` failed with `File has been modified since read, either by the user or by a linter. Read it again before attempting to write it.` The existing PreToolUse reminder only emphasized files that had not been read yet, so it did not proactively steer agents to refresh a stale snapshot before writing.

- [x] v3.3.9-1. **Preventive prompt widened** — `scripts/pre-tool-enforcer.mjs` now tells agents to `Read/re-read` existing files before `Write`/`Edit` when the file is unread, stale, or modified since read. New-file writes remain exempt.
- [x] v3.3.9-2. **Regression test** — `scripts/pre-tool-enforcer.test.mjs` adds `R-I2`, which verifies the existing-file reminder includes the exact target path in context and explicitly covers stale/modified-since-read prevention. RED before the prompt update, GREEN after.
- [x] v3.3.9-3. **Verification** — `node scripts/pre-tool-enforcer.test.mjs` passes: 68/68.

## Phase v3.3.4 — remove dead `current_stage === 'done'` branches (2026-04-23)

## Phase v3.3.6 — allow review-stage producer rework before explicit fail (2026-05-13)

- [x] v3.3.6-1. **PreToolUse Skill chain gate** — `scripts/state-contract-injector.mjs` now allows a `*-review` stage to re-enter its immediate producer before an explicit fail event exists. This keeps `workflow-investigate-review → workflow-investigate` available for supplemental evidence such as DB/API confirmation while preserving forward-skip blocking and `workflow-human-check` escape behavior.
- [x] v3.3.6-2. **Regression coverage** — `scripts/state-contract-injector.test.mjs` adds CE9b for review rework without a fail event; CE9 still pins explicit fail-route behavior.

## Phase v3.3.8 — bridge native AskUserQuestion answers into continuation (2026-05-13)

- [x] v3.3.8-1. **Transcript answer detection** — `scripts/lib/transcript-reader.mjs` now detects a pending native `AskUserQuestion` answer when the newest role entry is a user `tool_result` and no assistant response follows it.
- [x] v3.3.8-2. **Stop hook continuation** — `scripts/auto-confirm.mjs` handles that pending answer before generic halt branches, queues `native_answer_continue`, and injects the user's answer back to the next turn so the agent executes instead of stopping after the answer.
- [x] v3.3.8-3. **Regression coverage** — `transcript-reader.test.mjs` pins pending-answer detection and stale-answer rejection; `auto-confirm.test.mjs` reproduces the `/explore` review-pass + native answer stop and expects a session-scoped continuation queue.
...
- [x] v3.3-5. **Tests** — `surface-extraction.test.mjs` swaps `typo`→`text`, adds `디자인`/`css` design assertions, adds legacy-dropped validator case. `workflow-loader.test.mjs` asserts `text`/`design` → `fix_simple`. Current counts are `fix_simple` 4 and `fix` 10. `workflow-scenarios.test.mjs` `TARGET_TYPE_ENUM` assertion updated. Stub fixture (`mode-classifier-stub.mjs`) uses `surfaceText` / `surfaceDesign`.
...
Scope note: superseded by v0.55 above — `bug_fix` now owns the default 10-skill `fix` pipeline with compact tasker/tasker-review records. Escalation behavior for `extend_existing`/`new_feature`/`refactor`/`migration` is unchanged.
...
- [x] v3.2-1. **`medium` pipeline removed + /fix pipelines renamed.** Historical: `/fix` collapsed to two pipelines, `fix` (8 skills) and `fix_simple` (4). Current counts are `fix` 10 and `fix_simple` 4 after v0.55 kept compact tasker records only on the default `fix` path. `extend_existing`/`new_feature`/`refactor`/`migration` all return `upgrade_required` for `/fix` → escalate.
...
|-----------------------|----------------|---------|
| 11 skills × split review buckets | Historical: 8 skills × global 2-round reviews | Superseded by v0.55: tasker + tasker-review returned as compact records while team-code-review remains eliminated. |
...
- [x] Reverted the codex-scoped `.claude-<hash>.json` path because Claude Code v2.1+ resolves its state file as `${CLAUDE_CONFIG_DIR}/.claude.json` (non-hashed, see commit `285251a`) — the hashed sidecar left `additionalModelOptionsCache` invisible to the model picker, so `claude-codex` launched with no Codex models selectable. `codex-for-claude-code/scripts/set-model-mode.mjs` now returns to `getCodexConfigDir() = ~/.claude-codex` + `getCodexClaudeJsonPath() = ${configDir}/.claude.json`, and `codex-for-claude-code/scripts/claude-wrapper.mjs` points `CLAUDE_CONFIG_DIR` at that isolated dir. Session sharing via `--resume` regresses to pre-`8441363` behavior; that trade-off is deliberate until `ensureCodexConfigDir` is extended to symlink `projects`/`sessions`.
