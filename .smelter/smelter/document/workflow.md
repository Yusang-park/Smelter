...
# Smelter Workflow — v0.6 Prompt-Injection Workflow

> Smelter v0.6 separates **UserMode**, **TaskType**, **Step**, and **Guard** while moving entry-time workflow activation from mandatory state seeding to prompt-injected context.
> Smelter-first runtime: Smelter owns workflow sequencing and lifecycle control. Smelter's Claude Code hook registration no longer installs workflow blocking/controller hooks (`UserPromptSubmit`, `PreToolUse`, `PostToolUse`, or `Stop` workflow controllers), so local hook blocks cannot interrupt Smelter-run workflows.
> Workflow topology remains documented in `modes/workflow.yaml` for Smelter workflows and skill prompts; Claude Code hooks are support-only by default.

---

## v0.6 Entry Contract

Smelter invokes Smelter workflows directly. The default Claude Code hook registration does not route commands/natural-language intent through `UserPromptSubmit`; command routing, task memory, and workflow state are owned by the Smelter workflow run.

`plan.md` is intentionally concise:
...
|---|-------|----------------|------------|
| 1b | External slash passthrough | `EXTERNAL_COMMANDS` / `EXTERNAL_SLASH_COMMAND_RE` | prompt starts with external host commands such as `/goal`, so workflow routing fails closed instead of seeding a new mode from the trailing natural language |
| 2 | Passthrough | `PASSTHROUGH_PATTERNS` (anchored) | entire prompt matches pure git/shell verb-first (e.g. `git commit`, `커밋해`, `push 좀`) |
| 2b | Local high-confidence intent | `classifyHighConfidenceReadOnlyIntent` / `classifyHighConfidenceEditIntent` | read-only VCS analysis such as branch-vs-main diff questions routes to `/explore`; terse implementation orders such as `select만 적용`, concrete rich-text editor UI requests, and feature/UI-surface modification requests such as `AI panel 기능 수정해줘` or `특정 기능 수정` route to `/implement` |
...
1. `scripts/pre-tool-enforcer.mjs` — PreToolUse Write/Edit guard on `.state.json` path. Agents cannot directly mutate state. Escape hatch `SMT_HOOK_WRITE=1` is reserved for hook scripts. For source-file edits, the hook imports `workflow-v4-guard.mjs` and enforces `task_type` + `step` when legacy state exists: `read/design/verify` cannot edit source, `write` can edit source only at `EXECUTE` with executable specification RED evidence (`events[].type === 'test_red'`, `events[].type === 'tdd_adopted'`, `events[].type === 'tdd_exempt'`, or legacy `test_cycles[]` fail evidence), `infra` can edit source only at `EXECUTE` after infra plan/review entry, and `freeform` can edit source only at `EXECUTE`. Without `.state.json`, an active write/infra/freeform prompt pointer authorizes source edits, but `status: done` or read-only modes do not. The hook resolves the nearest ancestor `.smt` root before reading active workflow pointers, so running tools from a nested repo/subdirectory reuses the parent workflow state instead of false-blocking and triggering diagnostic loops. Both source-edit and commit gates read the session-scoped pointer first and then the explicit unscoped fallback (`session_id: ""`) so a later session id cannot bypass an active pointer-only workflow. It still enforces the **commit gate** for write/infra/freeform work: a Bash `git commit` is blocked unless active legacy state shows a `workflow-human-check` pass event / `completed_stages` includes `workflow-human-check`, and active pointer-only write/infra/freeform workflows are blocked until human-check passes. A `current_stage === 'done'` value is NOT a pass shape.
...
- producer re-entry before a verdict: allowed for review-stage rework or supplemental evidence; it does not mark the review failed.
...
**MANDATORY:** ask via the Claude Code `AskUserQuestion` tool (native clickable prompt), never as a plain-text menu. `AskUserQuestion` is a deferred tool — load its schema first via `ToolSearch("select:AskUserQuestion")`, then invoke it. Plain-text menus like `Decide: complete / rework / hold / upgrade?` are invalid; the Stop hook re-enters `workflow-human-check` and requires native `AskUserQuestion`. The same rule applies to `/explore` exit options (`commands/explore.md`): plain-text mode tables/lists such as `/implement`, `/brainstorm`, `continue /explore`, and `done` are rejected and re-injected as a native `AskUserQuestion` requirement.

Issue-first gate: before completion options, inspect `verify_report.md`, `agent-review.md`, `e2e-review.md`, `team_review.md`, and `results.md` when present. Any fail verdict, missing required evidence, unchecked acceptance item, `MEDIUM|HIGH|CRITICAL` risk, or user-visible blocker must be summarized with anchors and asked about via a problem-focused `AskUserQuestion`. In that question, do not offer `complete-commit-current` or `complete-new-branch-pr`; the only options are `rework`, `hold`, and `upgrade`. This applies to all modes, including a fresh `/verify` run after an earlier human-check.

Completion options, allowed only when no unresolved findings remain:
...
- Fresh native `AskUserQuestion` answer present as a pending `tool_result` in the transcript → drop a continuation payload before generic halt rules such as `/explore` review-pass halt. Native answers are tool results, not normal user-text prompts, so the Stop hook must explicitly bridge them into `auto-confirm-queue-<sessionId>.json`; otherwise the session can stop immediately after the user answers.
...
**R07 canonical artifact exemption:** workflow artifacts whose basename matches a registered workflow skill output (`investigation.md`, `investigation-review.md`, `tasks.md`, review files, etc.) are exempt from plan queue membership checks when written under either `.smt/features/<slug>/task/` or `.smt/features/<slug>/artifacts/`. This keeps artifact recovery writes from being treated as source-scope drift.
