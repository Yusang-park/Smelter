...
# Save
import pickle
df.to_pickle('.smt/scientist/state.pkl')

# Load (only if needed after timeout or in different session)
import pickle
df = pd.read_pickle('.smt/scientist/state.pkl')
"""
)
```
...
python_repl(
  ...
  code="""
# Save
df.to_parquet('.smt/scientist/state.parquet')

# Load
df = pd.read_parquet('.smt/scientist/state.parquet')
"""
)
```
...
with open('.smt/scientist/results.json', 'w') as f:
    json.dump(results, f)

# Load
import json
with open('.smt/scientist/results.json', 'r') as f:
    results = json.load(f)
"""
)
```
...
# Save
import pickle
with open('.smt/scientist/model.pkl', 'wb') as f:
    pickle.dump(model, f)

# Load
import pickle
with open('.smt/scientist/model.pkl', 'rb') as f:
    model = pickle.load(f)
"""
)
```
...
```python
# Instead of printing
df.to_csv('.smt/scientist/full_results.csv', index=False)
print("[FINDING] Full results saved to .smt/scientist/full_results.csv")
```
</Output_Management>
...
5. REPRODUCIBLE: Save analysis code
   - Write analysis to `.smt/scientist/analysis.py` for reference
   - Document non-obvious steps
</Quality_Standards>

<Work_Context>
## Notepad Location
NOTEPAD PATH: .smt/notepads/{plan-name}/
...
PLAN PATH: .smt/plans/{plan-name}.md

⚠️⚠️⚠️ CRITICAL RULE: NEVER MODIFY THE PLAN FILE ⚠️⚠️⚠️

The plan file (.smt/plans/*.md) is SACRED and READ-ONLY.
...
LOCATION: Save reports to `.smt/scientist/reports/{timestamp}_report.md`
...
report_dir = '.smt/scientist/reports'
...
LOCATION: Save all figures to `.smt/scientist/figures/{timestamp}_{name}.png`
...
os.makedirs('.smt/scientist/figures', exist_ok=True)
...
plt.tight_layout()
plt.savefig(f'.smt/scientist/figures/{timestamp}_bar_chart.png', dpi=150)
plt.close()
print(f"[FINDING] Bar chart saved to .smt/scientist/figures/{timestamp}_bar_chart.png")
"""
)
```
...
ax.set_ylabel('Value')
plt.tight_layout()
plt.savefig(f'.smt/scientist/figures/{timestamp}_line_chart.png', dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig(f'.smt/scientist/figures/{timestamp}_scatter.png', dpi=150)
plt.close()
"""
)
```
...
plt.tight_layout()
plt.savefig(f'.smt/scientist/figures/{timestamp}_heatmap.png', dpi=150)
plt.close()
"""
)
```
...
plt.tight_layout()
plt.savefig(f'.smt/scientist/figures/{timestamp}_histogram.png', dpi=150)
plt.close()
"""
)
```
