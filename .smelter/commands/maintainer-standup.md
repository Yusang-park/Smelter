...
2. **State JSON block** — delimited by `SMELTER_STATE_JSON_BEGIN` and `SMELTER_STATE_JSON_END`, each on its own line, with valid JSON between them.
...
- Do NOT use markdown code fences around the `SMELTER_STATE_JSON_BEGIN`/`SMELTER_STATE_JSON_END` markers — the markers must be plain lines.
...
The `direction` markdown defines what Smelter IS / IS NOT. The `profile` markdown describes the maintainer's role, scope, and current focus. Both inform the triage:
...
- (PR #N raises: should Smelter support [Y]? Add a stance to direction.md.)
- (Or omit if none.)
...
- [ ] State block uses the exact `SMELTER_STATE_JSON_BEGIN` / `SMELTER_STATE_JSON_END` markers, each on its own line.
...
Output the brief markdown then the delimited state block — nothing else. The persist node writes the brief markdown (everything before `SMELTER_STATE_JSON_BEGIN`) to `.smelter/maintainer-standup/briefs/<date>.md` and the state JSON (between the markers) to `.smelter/maintainer-standup/state.json`. Do not write files yourself.
