...
- `direction` — the project's committed direction.md (what Smelter IS / IS NOT, open questions)
...
- **aligned**: PR clearly fits one of the "What Smelter IS" clauses, or extends an existing pattern.
- **conflict**: PR clearly violates a "What Smelter is NOT" clause. Cite the specific clause (e.g. `direction.md §single-developer-tool`).
...
Unfortunately this isn't a direction we're taking with Smelter. Specifically, this conflicts with `direction.md §<clause>`: <one-sentence quote or paraphrase from the clause>.
...
For context, the project's stated scope lives at [`.smelter/maintainer-standup/direction.md`](../blob/dev/.smelter/maintainer-standup/direction.md). Open questions there are fair game for proposals — feel free to raise an issue if you'd like to push for a direction change.
```
