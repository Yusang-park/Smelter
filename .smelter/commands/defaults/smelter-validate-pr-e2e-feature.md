---
description: Start Smelter from the feature branch, use agent-browser to verify the fix works correctly
argument-hint: (none - reads from artifacts)
---

# E2E Testing: Feature Branch (Verify Fix)

Start Smelter from the **feature branch** (this worktree) and use browser automation to verify that the bug is fixed and the UI/UX is correct. Take screenshots as evidence.
...
- `taskkill //F //IM node.exe` or `taskkill //F //IM bun.exe` — this kills Claude Code, the Smelter server, and all other workflows
...
## Phase 2: Start Smelter on Feature Branch
...
cd "$WORKTREE_PATH" && PORT=$BACKEND_PORT bun run --filter @smelter/server dev > "$ARTIFACTS_DIR/.e2e-feature-backend.log" 2>&1 &
BACKEND_PID=$!
...
```bash
# 1. Open the Smelter UI (ALWAYS use --session)
FRONTEND_PORT=$(cat $ARTIFACTS_DIR/.frontend-port | tr -d '\n')
...
### 4.2 Stop Feature Branch Smelter (Cross-Platform)
...
- **SMELTER_STARTED**: Backend and frontend running on feature branch code
...
- **SMELTER_STOPPED**: Processes killed, ports freed — **VERIFY ports are free before finishing**
