...
Continue to `smelter-implement-tasks` to execute the plan.

{If STOP:}
Revise the plan to address blockers, then re-run `smelter-plan-setup`.
```
...
Proceed to `smelter-implement-tasks` to execute the plan.
```
...
3. **Re-run planning** with `/smelter-plan` to generate a fresh plan
