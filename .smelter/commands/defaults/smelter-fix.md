...
Backed by the Smelter-native `smelter-fix` workflow. The default fix path records a compact `tasks.md` checklist before coding. Extensions / new features / refactors escalate out of `/fix` to `/implement` or `/brainstorm`.
...
- No retry; failures route through Smelter node rework (Iron Law #4)
