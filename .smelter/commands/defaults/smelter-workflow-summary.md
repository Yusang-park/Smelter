...
**Reply with**: `@smelter do quick wins` to auto-fix these.
...
**Reply with**: `@smelter create follow-up issues` to create these.
...
**Reply with**: `@smelter update docs` to send a docs agent.
...
1. **Review the PR**: {url}
2. **Quick wins**: Reply `@smelter do quick wins` on PR (or skip)
3. **Create issues**: Reply `@smelter create follow-up issues` (or skip)
4. **Merge when ready**
