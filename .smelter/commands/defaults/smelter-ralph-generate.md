...
| `.smelter/ralph/{slug}/prd.md` exists | Existing PRD in ralph dir | Generate prd.json alongside it |
...
```bash
mkdir -p .smelter/ralph/{slug}
```

### 5.3 Write prd.md

**Output path**: `.smelter/ralph/{slug}/prd.md`
...
**Output path**: `.smelter/ralph/{slug}/prd.json`
...
```bash
git add .smelter/ralph/{slug}/
git commit -m "docs: add Ralph PRD for {feature name}"
```

**PHASE_5_CHECKPOINT:**
- [ ] `.smelter/ralph/{slug}/prd.md` written with all sections
- [ ] `.smelter/ralph/{slug}/prd.json` written with properly sized stories
...
- **FILES_WRITTEN**: Both prd.md and prd.json exist in `.smelter/ralph/{slug}/`
