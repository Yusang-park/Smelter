...
- `/fix`는 테스트/코딩 전에 compact `tasks.md`, `tasks-review.md` 범위 체크리스트를 기록합니다.
...
따라서 read/design/verify 모드는 source를 수정할 수 없고, write 모드는 RED 테스트 증거 전 product code를 수정할 수 없으며, commit은 `workflow-human-check` 통과 전 차단됩니다.
...
- 구현 작업은 테스트 먼저 작성, RED 확인, 구현, GREEN 확인 순서를 지킵니다.
